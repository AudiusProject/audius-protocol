...
creator_nodes = set()
for user in users:
    creator_nodes |= set(creator_node['endpoint']
...
synced_rects = ax.barh(y + width / 2, [len(synced[i]) for i in creator_nodes],
                       width,
                       label='Synced')
remaining_rects = ax.barh(y - width / 2,
                          [len(remaining[i]) for i in creator_nodes],
                          width,
                          label='Remaining')
...
y, width = np.arange(len(creator_nodes)), 0.3
total_rects = ax.barh(y + width / 2, [total[i] for i in creator_nodes],
                      width,
                      label='Total')
as_primary_rects = ax.barh(y - width / 2,
                           [as_primary[i] for i in creator_nodes],
                           width,
                           label='As primary')
