...
contract UserReplicaSetManager is SigningLogicInitializable, RegistryContract {
    ...
    ) external
    {
        ...
        require(
            userReplicaSets[_userId].primaryId == _oldPrimary,
    ...
    ) internal view returns (bool)
    {
        ...
        require(
            _oldSecondaries.length == userReplicaSets[_userId].secondaryIds.length,
            "Invalid prior secondary configuration"
        );
        bool secondarySenderFound = senderFound;
        for (uint i = 0; i < _oldSecondaries.length; i++) {
            require(
                userReplicaSets[_userId].secondaryIds[i] == _oldSecondaries[i],
                "Invalid prior secondary configuration"
            );
