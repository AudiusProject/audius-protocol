...
import {
  getUserFromFactory,
  getNetworkIdForContractInstance
} from '../utils/getters'
...
export const updateReplicaSet = async (
  ...
  let secondariesHash = web3New.utils.soliditySha3(
    web3New.eth.abi.encodeParameter(
      'uint[]',
      secondaries.map((x) => x.toNumber())
    )
  )
  let oldSecondariesHash = web3New.utils.soliditySha3(
    web3New.eth.abi.encodeParameter(
      'uint[]',
      oldSecondaries.map((x) => x.toNumber())
    )
  )
  let signatureData =
  ...
  let tx = await userReplicaSetManager.updateReplicaSet(
    userId,
    primary,
    secondaries,
    oldPrimary,
  ...
  parseTxWithAssertsAndResp(tx, 'UpdateReplicaSet', {
    _userId: userId,
    _primaryId: primary,
    _secondaryIds: secondaries
  })
  return tx
}
 ...
 */
export const addUserAndValidate = async (
  userFactory,
  userId,
  userWallet,
  multihashDigest,
  ...
  let signatureData = signatureSchemas.generators.getAddUserRequestData(
    chainId,
    userFactory.address,
    handle,
    nonce
  ...
  signatureData = signatureSchemas.generators.getUpdateUserMultihashRequestData(
    chainId,
    userFactory.address,
    userId,
    multihashDigest,
  ...
  let metadataTx = await userFactory.updateMultihash(
    userId,
    multihashDigest,
    nonce,
    sig
  )
  parseTxWithAssertsAndResp(metadataTx, 'UpdateMultihash', {
    _userId: userId,
    _multihashDigest: multihashDigest
  })
  signatureData = signatureSchemas.generators.getUpdateUserNameRequestData(
    chainId,
    userFactory.address,
    userId,
    userMetadata.name,
  ...
  sig = await eth_signTypedData(userWallet, signatureData)
  let nameTx = await userFactory.updateName(
    userId,
    userMetadata.name,
    nonce,
    sig
  )
  parseTxWithAssertsAndResp(nameTx, 'UpdateName', {
    _userId: userId,
    _name: userMetadata.name
  })
  signatureData = signatureSchemas.generators.getUpdateUserLocationRequestData(
    chainId,
    userFactory.address,
    userId,
  ...
  sig = await eth_signTypedData(userWallet, signatureData)
  let locationTx = await userFactory.updateLocation(
    userId,
    userMetadata.location,
    nonce,
    sig
  )
  parseTxWithAssertsAndResp(locationTx, 'UpdateLocation', {
    _userId: userId,
    _location: userMetadata.location
  })
  signatureData = signatureSchemas.generators.getUpdateUserBioRequestData(
    chainId,
    userFactory.address,
    userId,
    userMetadata.bio,
  ...
  parseTxWithAssertsAndResp(bioTx, 'UpdateBio', {
    _userId: userId,
    _bio: userMetadata.bio
  })
  signatureData =
    signatureSchemas.generators.getUpdateUserProfilePhotoRequestData(
      ...
      userFactory.address,
      userId,
      userMetadata.profilePhotoDigest,
      nonce
    )

  sig = await eth_signTypedData(userWallet, signatureData)
  let profilePhotoTx = await userFactory.updateProfilePhoto(
    userId,
    userMetadata.profilePhotoDigest,
    nonce,
    sig
  )
  parseTxWithAssertsAndResp(profilePhotoTx, 'UpdateProfilePhoto', {
    _userId: userId,
    _profilePhotoDigest: userMetadata.profilePhotoDigest
  })
  signatureData =
    signatureSchemas.generators.getUpdateUserCoverPhotoRequestData(
      ...
      userFactory.address,
      userId,
      userMetadata.coverPhotoDigest,
      nonce
    )

  sig = await eth_signTypedData(userWallet, signatureData)
  let coverPhotoTx = await userFactory.updateCoverPhoto(
    userId,
    userMetadata.coverPhotoDigest,
    nonce,
    sig
  )
  parseTxWithAssertsAndResp(coverPhotoTx, 'UpdateCoverPhoto', {
    _userId: userId,
    _coverPhotoDigest: userMetadata.coverPhotoDigest
  })
  signatureData = signatureSchemas.generators.getUpdateUserCreatorRequestData(
    chainId,
    userFactory.address,
    userId,
    isCreator,
  ...
  sig = await eth_signTypedData(userWallet, signatureData)
  let isCreatorTx = await userFactory.updateIsCreator(
    userId,
    isCreator,
    nonce,
    sig
  )
  parseTxWithAssertsAndResp(isCreatorTx, 'UpdateIsCreator', {
    _userId: userId,
  ...
  sig = await eth_signTypedData(userWallet, signatureData)
  let creatorNodeEndpointTx = await userFactory.updateCreatorNodeEndpoint(
    userId,
    newCnode,
    nonce,
    sig
  ...
  parseTxWithAssertsAndResp(
    creatorNodeEndpointTx,
    'UpdateCreatorNodeEndpoint',
    { _userId: userId, _creatorNodeEndpoint: newCnode }
  )
...
export const updateUserNameAndValidate = async function (
  userFactory,
  userId,
  userWallet
) {
  const nonce = signatureSchemas.getNonce()
  const chainId = getNetworkIdForContractInstance(userFactory) // in testing use the network id as chain ID because chain ID is unavailable
  let signatureData = signatureSchemas.generators.getUpdateUserNameRequestData(
    chainId,
    userFactory.address,
    userId,
    userMetadata.name,
...
export const markUserVerifiedAndValidate = async function (
  userFactory,
  verifierWallet,
  userId,
  verified
  ...
  let verifiedTx = await userFactory.updateIsVerified(
    userId,
    verified,
    nonce,
    sig
  ...
  return parseTxWithAssertsAndResp(verifiedTx, 'UpdateIsVerified', {
    _userId: userId,
 ...
 */
export const addUserFollowAndValidate = async (
  ...
  let tx = await socialFeatureFactory.addUserFollow(
    followerUserId,
    followeeUserId,
    nonce,
    sig
  ...
  let event = parseTxWithResp(tx, {
    _followerUserId: true,
    _followeeUserId: true
  })
  validateObj(event, {
    eventName: 'UserFollowAdded',
    followerUserId,
    followeeUserId
  })
  return {
 ...
 * @returns {object} with event data
 */
export const deleteUserFollowAndValidate = async (
  socialFeatureFactory,
  userWallet,
  followerUserId,
  followeeUserId
  ...
  let tx = await socialFeatureFactory.deleteUserFollow(
    followerUserId,
    followeeUserId,
    nonce,
    sig
