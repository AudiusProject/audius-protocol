...
export const addPlaylistAndValidate = async (
  playlistFactory,
  expectedPlaylistId,
  walletAddress,
  playlistOwnerId,
  ...
  let trackIdsHash = web3New.utils.soliditySha3(
    web3New.eth.abi.encodeParameter('uint[]', trackIds)
  )
  const signatureData =
  ...
  assert.equal(
    returnedPlaylistId,
    expectedPlaylistId,
    'Expected playlist id does not match'
  )
  ...
  for (let i = 0; i < emittedTrackIds.length; i++) {
    ...
    assert.equal(
      emittedTrackId,
      trackIds[i],
      'Expected ordered event trackId to match input'
    )
  }

  // Validate storage playlist contents
  for (let i = 0; i < trackIds; i++) {
    let isTrackInPlaylist = await playlistFactory.isTrackInPlaylist.call(
      expectedPlaylistId,
      trackIds[i]
    )
    assert.equal(
      isTrackInPlaylist,
      true,
      'Expected all tracks to be added to playlist'
    )
  }
}

export const deletePlaylistAndValidate = async (
  playlistFactory,
  walletAddress,
  playlistId
) => {
...
export const orderPlaylistTracksAndValidate = async (
  playlistFactory,
  walletAddress,
  playlistId,
  trackIds
...
export const updatePlaylistPrivacyAndValidate = async (
  playlistFactory,
  walletAddress,
  playlistId,
...
export const updatePlaylistNameAndValidate = async (
  playlistFactory,
  walletAddress,
  playlistId,
  updatedPlaylistName
...
export const updatePlaylistCoverPhotoAndValidate = async (
  playlistFactory,
  walletAddress,
  playlistId,
...
export const updatePlaylistUPCAndValidate = async (
  playlistFactory,
  walletAddress,
  playlistId,
  updatedPlaylistUPC
...
export const updatePlaylistDescriptionAndValidate = async (
  playlistFactory,
  walletAddress,
  playlistId,
 ...
 */
export const addPlaylistRepostAndValidate = async (
  ...
  let tx = await socialFeatureFactory.addPlaylistRepost(
    userId,
    playlistId,
    nonce,
    sig
  ...
  // validate storage
  let isPlaylistReposted = await socialFeatureFactory.userRepostedPlaylist.call(
    userId,
    playlistId
  )
  assert.isTrue(
    isPlaylistReposted,
    'Expect storage to confirm added playlist repost'
  )
 ...
 * @returns {object} with event data
 */
export const deletePlaylistRepostAndValidate = async (
  socialFeatureFactory,
  userAddress,
  userId,
  playlistId
  ...
  let tx = await socialFeatureFactory.deletePlaylistRepost(
    userId,
    playlistId,
    nonce,
    sig
  ...
  assert.isFalse(
    isPlaylistReposted,
    'Expect storage to confirm deleted playlist repost'
  )
...
export const addPlaylistTrack = async (
  ...
  assert.isTrue(
    isTrackInPlaylist,
    'Expected newly added tracks to be in playlist'
  )
...
export const deletePlaylistTrack = async (
  ...
  assert.isFalse(
    isTrackInPlaylist,
    'Expected newly deleted tracks to not be in playlist'
  )

  return parsedDeletePlaylistTrack
}

export const addPlaylistSaveAndValidate = async (
  userLibraryFactory,
  userAddress,
  userId,
  playlistId
  ...
  assertEqualValues(parsedPlaylistSave, undefined, {
    _userId: eventInfo._userId,
    _playlistId: eventInfo._playlistId
  })
}

export const deletePlaylistSaveAndValidate = async (
  userLibraryFactory,
  userAddress,
  userId,
  playlistId
