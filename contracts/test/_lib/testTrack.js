 ...
 */
export const addTrackAndValidate = async (
  trackFactory,
  trackId,
  walletAddress,
  trackOwnerId,
  ...
  let tx = await trackFactory.addTrack(
    trackOwnerId,
    multihashDigest,
    multihashHashFn,
    multihashSize,
  ...
  let event = parseTxWithResp(tx, {
    _id: true,
    _trackOwnerId: true,
    _multihashDigest: true,
    _multihashHashFn: true,
    _multihashSize: true
  })
  validateObj(event, {
    eventName: 'NewTrack',
 ...
 */
export const deleteTrackAndValidate = async (
  trackFactory,
  walletAddress,
  trackId
) => {
...
export const updateTrack = async (
  ...
  const sig = await eth_signTypedData(walletAddress, signatureData)
  return trackFactory.updateTrack(
    trackId,
    trackOwnerId,
    multihashDigest,
    multihashHashFn,
 ...
 * @returns {object} with event data
 */
export const addTrackRepostAndValidate = async (
  ...
  let tx = await socialFeatureFactory.addTrackRepost(
    userId,
    trackId,
    nonce,
    sig
  ...
  // validate storage
  let isTrackReposted = await socialFeatureFactory.userRepostedTrack.call(
    userId,
    trackId
  )
 ...
 * @returns {object} with event data
 */
export const deleteTrackRepostAndValidate = async (
  socialFeatureFactory,
  userAddress,
  userId,
  trackId
  ...
  let tx = await socialFeatureFactory.deleteTrackRepost(
    userId,
    trackId,
    nonce,
    sig
  ...
  assert.isFalse(
    isTrackReposted,
    'Expect storage to confirm added track repost'
  )
  return {
    event: event
  }
...
export const addTrackSaveAndValidate = async (
  userLibraryFactory,
  userAddress,
  userId,
  trackId
  ...
  assertEqualValues(parsedTrackSave, undefined, {
    _userId: eventInfo._userId,
    _trackId: eventInfo._trackId
  })
}

export const deleteTrackSaveAndValidate = async (
  userLibraryFactory,
  userAddress,
  userId,
  trackId
  ...
  assertEqualValues(parsedDeleteTrackSave, undefined, {
    _userId: eventInfo._userId,
    _trackId: eventInfo._trackId
  })
}
