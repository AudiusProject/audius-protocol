...
contract('EntityManager', async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    entityManagerContract = await EntityManager.at(
  ...
  it('Verifier basic test', async () => {
    ...
    let verifyTx = await entityManagerContract.manageIsVerified(1, true, {
  ...
  it('EntityManager upgradeability validation', async () => {
    ...
    let deployUpgradedLogicContract = await TestEntityManager.new({
      from: deployer
    })
    let proxyInstance = await AdminUpgradeabilityProxy.at(
      entityManagerContract.address
    )
    ...
    let testEntityManager = await TestEntityManager.at(
      entityManagerContract.address
    )
    ...
    let verifyTx = await testEntityManager.manageIsVerified(1, true, {
      from: verifierAddress
    })
    await expectEvent.inTransaction(
      verifyTx.tx,
