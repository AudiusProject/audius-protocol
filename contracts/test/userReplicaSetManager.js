...
contract("UserReplicaSetManager", async (accounts) => {
  ...
  const bootstrapDelegateWallets = [
    cnode1DelegateOwnerWallet,
    cnode2DelegateOwnerWallet,
    cnode3DelegateOwnerWallet,
    cnode4DelegateOwnerWallet,
  ];
  const bootstrapOwnerWallets = [
    cnode1OwnerWallet,
    cnode2OwnerWallet,
    cnode3OwnerWallet,
    cnode4OwnerWallet,
  ...
  beforeEach(async () => {
    ...
    userReplicaSetManager = await UserReplicaSetManager.at(
      proxyContractDeployTx.address
    );

    let seedComplete = await userReplicaSetManager.getSeedComplete({
    ...
    seedComplete = await userReplicaSetManager.getSeedComplete({
      from: userReplicaBootstrapAddress,
    });
    ...
    event1ProposerSpIds.forEach((x) => {
      assert.isTrue(x.eq(toBN(0)));
    });
    event2ProposerSpIds.forEach((x) => {
      assert.isTrue(x.eq(toBN(0)));
    });
    event3ProposerSpIds.forEach((x) => {
      assert.isTrue(x.eq(toBN(0)));
    });
    event4ProposerSpIds.forEach((x) => {
      assert.isTrue(x.eq(toBN(0)));
    });
  });
  ...
  const validateBootstrapNodesInternal = async (contractInstance) => {
    ...
    for (var i = 0; i < bootstrapSPIds.length; i++) {
      ...
      let walletInfoFromChain = await contractInstance.getContentNodeWallets(
        spID
      );
      assert.isTrue(
  ...
  const toBNArray = (bnArray) => {
    return bnArray.map((x) => toBN(x));
  };

  let updateReplicaSet = async (
    userId,
    newPrimary,
    newSecondaries,
    oldPrimary,
    ...
    let replicaSetFromChain = await userReplicaSetManager.getUserReplicaSet(
      userId
    );
    assert.isTrue(
      replicaSetFromChain.primaryId.eq(newPrimary),
      "Primary mismatch"
    );
    assert.isTrue(
      replicaSetFromChain.secondaryIds.every((replicaId, i) =>
        replicaId.eq(newSecondaries[i])
      ),
      "Secondary mismatch"
    );
    await expectEvent.inTransaction(
  ...
  it("Validate constructor bootstrap arguments", async () => {
    ...
    let localUrsmContract = await UserReplicaSetManager.at(
      unSeededProxy.address
    );
    // Confirm unseeded contract rejects operations
    await expectRevert(
      localUrsmContract.updateUserReplicaBootstrapAddress(accounts[20], {
        from: userReplicaBootstrapAddress,
      }),
      "Must be initialized"
    );
    ...
    const invalidBootstrapOwnerWallets = [
      cnode1OwnerWallet,
      cnode2OwnerWallet,
      cnode4OwnerWallet,
    ];
  ...
  it("Register additional nodes w/multiple signers (bootstrap nodes)", async () => {
    ...
    const chainIdForContract = getNetworkIdForContractInstance(
      userReplicaSetManager
    );
    ...
    let walletInfoFromChain = await userReplicaSetManager.getContentNodeWallets(
      newCNodeSPId
    );
    let newDelegateWalletFromChain = walletInfoFromChain.delegateOwnerWallet;
    assert.equal(
      newDelegateWalletFromChain,
      newCnodeDelegateWallet,
      "Expect delegatOwnerWallet assignment"
    );
    let newOwnerWalleFromChain = walletInfoFromChain.ownerWallet;
    assert.equal(
      newOwnerWalleFromChain,
      newCnodeOwnerWallet,
      "Expect delegatOwnerWallet assignment"
    );
    await expectEvent.inTransaction(
