...
[here](/token/running-a-node/setup/installation#creator-node).
...
There are two required discovery provider environment variables, available in the discovery provider
section [here](/token/running-a-node/setup/installation#discovery-provider).
