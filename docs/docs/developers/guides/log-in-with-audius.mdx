...
See complete instructions [here](/developers/sdk/#install-the-sdk) to install and initialize the
JavaScript SDK.
...
[`renderButton`](/developers/sdk-oauth-methods#renderbutton) replaces the given `element` with the
Log In with Audius button.
...
login popup with [`audiusSdk.oauth.login`](/developers/sdk-oauth-methods#login).
...
See [Getting Started](/developers/sdk) with the SDK or
