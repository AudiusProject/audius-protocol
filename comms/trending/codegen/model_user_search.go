/*
API

Audius V1 API

API version: 1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package codegen

import (
	"encoding/json"
)

// checks if the UserSearch type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UserSearch{}

// UserSearch struct for UserSearch
type UserSearch struct {
	Data []User `json:"data,omitempty"`
}

// NewUserSearch instantiates a new UserSearch object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUserSearch() *UserSearch {
	this := UserSearch{}
	return &this
}

// NewUserSearchWithDefaults instantiates a new UserSearch object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUserSearchWithDefaults() *UserSearch {
	this := UserSearch{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *UserSearch) GetData() []User {
	if o == nil || IsNil(o.Data) {
		var ret []User
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UserSearch) GetDataOk() ([]User, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *UserSearch) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []User and assigns it to the Data field.
func (o *UserSearch) SetData(v []User) {
	o.Data = v
}

func (o UserSearch) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UserSearch) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	return toSerialize, nil
}

type NullableUserSearch struct {
	value *UserSearch
	isSet bool
}

func (v NullableUserSearch) Get() *UserSearch {
	return v.value
}

func (v *NullableUserSearch) Set(val *UserSearch) {
	v.value = val
	v.isSet = true
}

func (v NullableUserSearch) IsSet() bool {
	return v.isSet
}

func (v *NullableUserSearch) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUserSearch(val *UserSearch) *NullableUserSearch {
	return &NullableUserSearch{value: val, isSet: true}
}

func (v NullableUserSearch) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUserSearch) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


