...
func TestMakeBuckets(t *testing.T) {
	testCases := map[string]struct {
		...
		"Prefix length 1 (a, b, ..., 8, 9)": {
			prefixLength: 1,
			expected:     base36Chars[:],
		},
		"Prefix length 2 (ab, bc, ..., 98, 99)": {
			prefixLength: 2,
	...
	for testName, testCase := range testCases {
		...
		result := New(testCase.prefixLength).Buckets
...
func TestGetBucketForId(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"'a' bucket": {
			bucketer:               prefixLength1,
			bucket:                 "a",
			idsExpectedInBucket:    []string{"aabcdefg"},
			idsExpectedNotInBucket: []string{"babcdefg", "4abcdefg", "xrbcdefg", "9sbcdefg"},
		},
		"'4' bucket": {
			bucketer:               prefixLength1,
			bucket:                 "4",
			idsExpectedInBucket:    []string{"4abcdefg", "444444444"},
			idsExpectedNotInBucket: []string{"aabcdefg", "babcdefg", "xrbcdefg", "9sbcdefg"},
		},
		"'xr' bucket": {
			bucketer:               prefixLength2,
			bucket:                 "xr",
			idsExpectedInBucket:    []string{"xrbcdefg"},
			idsExpectedNotInBucket: []string{"aabcdefg", "babcdefg", "4abcdefg", "9sbcdefg"},
		},
		"'9s' bucket": {
			bucketer:               prefixLength2,
			bucket:                 "9s",
			idsExpectedInBucket:    []string{"9sbcdefg"},
			idsExpectedNotInBucket: []string{"aabcdefg", "babcdefg", "4abcdefg", "xrbcdefg"},
		},
	}
