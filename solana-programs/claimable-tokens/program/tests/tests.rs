...
async fn transfer_nonce_account_denial_with_lamports() {
    ...
    program_context.banks_client.process_transaction(send_lamports_transaction).await.unwrap();
    ...
    let user_token_account_amount =
        spl_token::state::Account::unpack(&user_token_account_data.data.as_slice()).unwrap().amount;
    ...
    // check that program sent required number tokens from bank token account to user token account
    assert_eq!(bank_token_account.amount, tokens_amount - 2 * transfer_amount);
