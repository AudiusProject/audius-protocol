...
# recreate Github Releases (non-prerelease) + Git Tags
${WORKDIR}/ghr \
  -token ${GH_TOKEN} \
...
cd ${WORKDIR}/audius-docker-compose
git pull
sed -i "s|\${TAG:-.*}|\${TAG:-${RELEASE_HASH}}|" \
  */docker-compose.yml
git add */docker-compose.yml
git commit -m "Bump to version ${RELEASE_VERSION}"
git push

# create and vote on governance upgrade proposal
cd ${WORKDIR}
