...
export class Web3Manager {
  ...
  constructor({
    ...
    // @ts-expect-error
    global.signTypedData = this.signTypedData
  }
  ...
  async sign(data: string | Buffer) {
    ...
    return sigUtil.personalSign({
      privateKey: this.getOwnerWalletPrivateKey()!,
      data
    })
  }
  ...
  async verifySignature(data: string, signature: string) {
    return sigUtil.recoverPersonalSignature({ data, signature })
  }

  async signTypedData(signatureData: TypedMessage<MessageTypes>) {
    if (this.useExternalWeb3) {
      ...
      return sigUtil.signTypedData({
        privateKey: this.ownerWallet!.getPrivateKey(),
        data: signatureData,
...
const ethSignTypedData = async (
  ...
  signatureData: TypedMessage<MessageTypes>
) => {
  return await new Promise((resolve, reject) => {
    let processedSignatureData: TypedMessage<MessageTypes> | string =
      signatureData
    let method
