...
export class CreatorNode {
  ...
  async uploadTrackContent(
    ...
    try {
      await this.uploadV2TrackContent(trackFile, coverArtFile, metadata, onProgress)
    } catch (e: any) {
      console.log('Error uploading track content', e)
    }
  ...
  async uploadV2TrackContent(
    ...
    const trackFileUploadResponse = await axios({
      ...
      url: 'http://node1-storage/storage/api/v1/file',
    ...
    console.log(`uploaded track: http://node1-storage/storage/search/${id}`)
