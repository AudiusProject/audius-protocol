...
export class ServiceSelector {
  ...
  /**
   * Services from this list should not be selected
   */
  protected blacklist: Set<string>
  /**
   * Select only services from this list
   */
  protected whitelist: Set<string>
  /**
   * An (async) method to get a list of services to choose from
   */
  protected getServices: () => Promise<Array<string>>
  /**
   * The maximum number of requests allowed to fire at once. Tweaking this may impact browser performance
   */
  protected maxConcurrentRequests: number
  /**
   * The timeout (in ms) to give up on a service
   */
  protected requestTimeout: number
  /**
   * Truly unhealthy services, Should never be selected.
   */
  protected unhealthy: Set<string>
  /**
   * How long services remain downgraded to "unhealthy" before being available to be retried for selection
   */
  protected unhealthyTTL: number
  private unhealthyCleanupTimeout: NodeJS.Timeout

  /**
   * Services that are selectable but not optimal. Will be selected as a last resort.
   */
  protected backups: Record<string, any>
  /**
   * How long services remain downgraded to "backup" before being available to be retried for selection
   */
  protected backupsTTL: number
  private backupCleanupTimeout: NodeJS.Timeout

  /**
   * Total number of services attempted
   */
  protected totalAttempts: number

  /**
   * The decision tree path that was taken. Reset on each new selection.
   */
  protected decisionTree: Array<DecisionTreeStage>
  ...
  async findAll () {
    ...
    const map = services.reduce((acc, s) => {
      acc[ServiceSelector.getHealthCheckEndpoint(s)] = s
      return acc
    }, {})
   ...
   * @param {any} response the services response. This can be used to weigh various
  ...
  async selectFromBackups () {
    return Promise.resolve(Object.keys(this.backups)[0])
  }
