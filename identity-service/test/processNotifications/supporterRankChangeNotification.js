...
describe('Test Supporter Rank Change Notification', function () {
  ...
  it('should insert rows into notifications and notifications actions tables', async function () {
    ...
    const processedNotifications = await processSupporterRankChangeNotification(
    ...
    assert.deepStrictEqual(processedNotifications[1], {
      ...
      initiator: 3, // Notif goes to the dethroned user
      metadata: {
        supportedUserId: 1, // The user originally tipped
        newTopSupporterUserId: 2, // The usurping user
        oldAmount: 9,
        newAmount: 11
      }
    })
