...
module.exports = function (app) {
  ...
  app.post('/relay', handleResponse(async (req, res, next) => {
    ...
    if (body && body.contractRegistryKey && body.contractAddress && body.senderAddress && body.encodedABI) {
      ...
      let receipt
      try {
        receipt = await txRelay.sendTransaction(body.contractRegistryKey, body.contractAddress, body.encodedABI, body.senderAddress)
      } catch (e) {
        if (e.message.includes('nonce')) {
          ...
          receipt = await txRelay.sendTransaction(body.contractRegistryKey, body.contractAddress, body.encodedABI, body.senderAddress, true)
