...
const StateMonitoringQueue = require('../src/services/stateMachineManager/stateMonitoring')
const {
  QUEUE_NAMES
...
describe.skip('test StateMonitoringQueue initialization and events', function () {
  let server, sandbox
  ...
  it('creates the queue and registers its event handlers', async function () {
    // Initialize StateMonitoringQueue and spy on its registerQueueEventHandlersAndJobProcessors function
    const stateMonitoringQueue = new StateMonitoringQueue()
    sandbox.spy(
      stateMonitoringQueue,
      'registerQueueEventHandlersAndJobProcessors'
    )
    const queue = await stateMonitoringQueue.init('discoveryNodeEndpoint')
    ...
    expect(stateMonitoringQueue.registerQueueEventHandlersAndJobProcessors).to
      .have.been.calledOnce
    expect(
      stateMonitoringQueue.registerQueueEventHandlersAndJobProcessors.getCall(0)
        .args[0]
    )
  ...
  it("doesn't queue or process any jobs when rate limit jobsPerInterval is set to 0", async function () {
    ...
    // Initialize StateMonitoringQueue
    const stateMonitoringQueue = new MockStateMonitoringQueue()
    const queue = await stateMonitoringQueue.init('discoveryNodeEndpoint')
...
describe.skip('test StateMonitoringQueue re-enqueuing', function () {
  let server, sandbox
  beforeEach(async function () {
    const appInfo = await getApp(getLibsMock())
    await appInfo.app.get('redisClient').flushdb()
