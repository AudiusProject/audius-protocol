...
class TranscodingQueue {
  constructor () {
    ...
    this.queue.process(PROCESS_NAMES.segment, MAX_CONCURRENCY, async (job, done) => {
      ...
      const { fileDir, fileName, logContext, useFFMPEG4 } = job.data
   ...
   * @param {object} logContext to create a logger.child(logContext) from
   */
  async segment (fileDir, fileName, { logContext, useFFMPEG4 }) {
    const job = await this.queue.add(
      PROCESS_NAMES.segment,
      { fileDir, fileName, logContext, useFFMPEG4 }
    )
   ...
   * @param {object} logContext to create a logger.child(logContext) from
   */
  async transcode320 (fileDir, fileName, { logContext, useFFMPEG4 }) {
    const job = await this.queue.add(
