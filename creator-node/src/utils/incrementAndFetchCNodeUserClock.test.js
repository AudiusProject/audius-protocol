...
const { incrementAndFetchCNodeUserClock } = require('./incrementAndFetchCNodeUserClock')
const utils = require('../utils')

describe('Test incrementAndFetchCNodeUserClock', () => {
  const req = {
    logger: {
  ...
  beforeEach(async () => {
    ...
    let cnodeUser = await models.CNodeUser.findOne({ where: { cnodeUserUUID: req.session.cnodeUserUUID } })
  ...
  it.only('Concurrent increment&Fetch', async () => {
    ...
    assert.deepEqual(returnedClockValsSorted, arr)
  })
