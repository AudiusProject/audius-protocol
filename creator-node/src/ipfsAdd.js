...
// TODO: remove ipfsLatest input, and then bake it into this fn
// then, make separate class to import ipfsClient
//

/*
  function ipfsAddNonImages(...) {
    try {
       // logic
      if (mismatch) throw e
    } catch (e) {
  ...
  = updte all unit tests to handle sad/happy path cases with the update in try/cathc logic as expected

*/
async function ipfsAddNonImages (content, ipfsConfig = {}, logContext = {}, enableIPFSAdd = false) {
...
/**
   ...
   */
async function ipfsAddImages (content, ipfsConfig = {}, logContext = {}, enableIPFSAdd = false) {
  const logger = genericLogger.child(logContext)
