...
class StateMonitoringQueueUtils {
  /**
   ...
   */
  static async getLatestUserId(discoveryNodeEndpoint) {
   ...
   */
  static buildReplicaSetNodesToUserWalletsMap(nodeUsers) {
  ...
  static async computeUserSecondarySyncSuccessRatesMap(nodeUsers) {
   ...
   */
  static async aggregateReconfigAndPotentialSyncOps(
    nodeUsers,
    unhealthyPeers,
    ...
    for (const nodeUserBatch of nodeUserBatches) {
      const resultBatch = await Promise.allSettled(
        nodeUserBatch.map((nodeUser) =>
          StateMonitoringQueueUtils._aggregateOps(
            nodeUser,
            unhealthyPeers,
...
module.exports = StateMonitoringQueueUtils
