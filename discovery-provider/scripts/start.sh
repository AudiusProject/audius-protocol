...
if [[ "$audius_discprov_dev_mode" == "true" ]]; then
    ...
    if [[ "$audius_no_workers" != "true" ]] && [[ "$audius_no_workers" != "1" ]]; then
        ...
        audius_service=beat celery -A src.worker.celery --loglevel WARNING --schedule=/var/celerybeat-schedule --pidfile=/var/celerybeat.pid beat 2>&1 | tee >(logger -t beat) &
        audius_service=worker watchmedo auto-restart --directory ./ --pattern=*.py --recursive -- celery -A src.worker.celery --loglevel $audius_discprov_loglevel worker 2>&1 | tee >(logger -t worker) &
    fi
else
    if [[ "$audius_no_server" != "true" ]] && [[ "$audius_no_server" != "1" ]]; then
    ...
    if [[ "$audius_no_workers" != "true" ]] && [[ "$audius_no_workers" != "1" ]]; then
        ...
        [ -e /var/celerybeat.pid ] && rm /var/celerybeat.pid
        audius_service=beat celery -A src.worker.celery --loglevel WARNING --schedule=/var/celerybeat-schedule --pidfile=/var/celerybeat.pid beat 2>&1 | tee >(logger -t beat) &
        audius_service=worker celery -A src.worker.celery --max-memory-per-child 300000 --loglevel $audius_discprov_loglevel worker 2>&1 | tee >(logger -t worker) &
    fi
fi
