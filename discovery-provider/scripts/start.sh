...
if [[ "$audius_discprov_dev_mode" == "true" ]]; then
    ...
    if [[ "$audius_no_workers" != "true" ]] && [[ "$audius_no_workers" != "1" ]]; then
        ...
        audius_service=beat celery -A src.worker.celery --schedule=/var/celerybeat-schedule --pidfile=/var/celerybeat.pid beat --loglevel WARNING 2>&1 | tee >(logger -t beat) &
    ...
    if [[ "$audius_no_workers" != "true" ]] && [[ "$audius_no_workers" != "1" ]]; then
        ...
        [ -e /var/celerybeat.pid ] && rm /var/celerybeat.pid
        audius_service=beat celery -A src.worker.celery --schedule=/var/celerybeat-schedule --pidfile=/var/celerybeat.pid beat --loglevel WARNING 2>&1 | tee >(logger -t beat) &
        audius_service=worker celery -A src.worker.celery --max-memory-per-child 300000 worker --loglevel $audius_discprov_loglevel 2>&1 | tee >(logger -t worker) &
    fi
fi
