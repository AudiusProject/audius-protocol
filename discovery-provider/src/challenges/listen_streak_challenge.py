...
from src.challenges.challenge import ChallengeManager, ChallengeUpdater, FullEventMetadata
...
class ListenStreakChallengeUpdater(ChallengeUpdater):
    ...
    def on_after_challenge_creation(self, session: Session, metadatas: List[FullEventMetadata]):
    ...
    # Helpers
    def _handle_track_listens(self, partial_completions: List[ListenStreakChallenge], event_metadatas: List[FullEventMetadata]):
        for idx, partial_completion in enumerate(partial_completions):
            last_date = partial_completion.last_listen_date
            new_date = datetime.fromtimestamp(event_metadatas[idx]["extra"]["created_at"])
...
# Accessors
def get_listen_streak_challenges(session: Session, user_ids: List[int]) -> List[ListenStreakChallenge]:
    return (
