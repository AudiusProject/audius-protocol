...
def update_track_is_available_in_db(db, tracks_with_updated_is_available_status):
    ...
    with db.scoped_session() as session:
        query_results = session.bulk_update_mappings(
            Track,
            tracks_with_updated_is_available_status
        )
...
def query_replica_set_by_track_id(db, track_ids):
    ...
    with db.scoped_session() as session:
        query_results = (
            session.query(
                Track.track_id, User.primary_id, User.secondary_ids
            )
...
def check_track_is_available(redis, track_id, spID_replica_set):
    ...
    while not is_available_in_network and i < len(spID_replica_set):
        spID_unavailable_tracks_key = get_unavailable_tracks_redis_key(spID_replica_set[i])
...
def update_track_is_available(self):
    ...
    for i in range(0, len(all_unavailable_track_ids), BATCH_SIZE):
        ...
        tracks_to_update = []
        track_ids_to_replica_set = query_replica_set_by_track_id(db, unavailable_track_ids_batch)

        for entry in track_ids_to_replica_set:
            track_id = entry[0]
            spID_replica_set = [entry[1], *entry[2]]
            is_available = check_track_is_available(redis=redis, track_id=track_id, spID_replica_set=spID_replica_set)
            tracks_to_update.append({"track_id": track_id, "is_available": is_available})
