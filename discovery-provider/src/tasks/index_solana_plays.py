...
def process_solana_plays(solana_client_manager: SolanaClientManager, redis: Redis):
    ...
    if latest_play_slot:
        logger.info(f"index_solana_plays.py | Setting latest plays slot {latest_play_slot}")
        redis.set(latest_sol_plays_slot_key, latest_play_slot)
    elif latest_global_slot is not None:
        logger.info(f"index_solana_plays.py | Setting latest plays slot as the latest global slot {latest_global_slot}")
