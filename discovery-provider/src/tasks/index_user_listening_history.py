...
def _index_user_listening_history(session):
    ...
    users_with_new_plays = set([new_play.user_id for new_play in new_plays])
    ...
    existing_users = set([user_history.user_id for user_history in existing_user_listening_history])
    ...
    for i, user_history in enumerate(existing_user_listening_history):
        ...
        for new_play in reversed(update_user_listening_history_dict[user_history.user_id]):
            deduped_history_dict[new_play["track_id"]] = new_play["timestamp"]
        existing_user_listening_history[i].listening_history = sort_listening_history(deduped_history_dict)
...
def index_user_listening_history(self):
    ...
    try:
        ...
        if have_lock:
            logger.info(f"index_user_listening_history.py | Updating {USER_LISTENING_HISTORY_TABLE_NAME}")
