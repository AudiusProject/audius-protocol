...
class IPFSClient:
    ...
    def get_metadata(self, multihash, default_metadata_fields, user_replica_set=None):
        ...
        """
        logger.warning(f"IPFSCLIENT  {self._id}| get_metadata - {multihash}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            ...
            for get_metadata_future in concurrent.futures.as_completed(
                ...
                try:
                    ...
                    if retrieved:
                        logger.info(
                            f"IPFSCLIENT  {self._id}| retrieved metadata successfully, \
                            {api_metadata}, \
                    ...
                    logger.error(
                        f"IPFSCLIENT  {self._id}| ipfs_lib.py | \
        ...
        if not retrieved_metadata:
            logger.error(
                f"IPFSCLIENT  {self._id}| Retrieved metadata: {retrieved_metadata}. "
            ...
            raise Exception(
                f"IPFSCLIENT  {self._id}| Failed to retrieve metadata. Using default values for {multihash}"
            )

        duration = time.time() - start_time
        logger.info(
            f"IPFSCLIENT  {self._id}| get_metadata ${multihash} {duration} seconds \
    ...
    def load_metadata_url(self, url, max_timeout):
        ...
        if not validate_url.scheme:
            raise Exception(
                f"IPFSCLIENT  {self._id}| Invalid URL from provided gateway addr - {url}"
            )
            ...
            for future in concurrent.futures.as_completed(future_to_url):
                ...
                try:
                    ...
                    if r.status_code != 200:
                        logger.warning(
                            f"IPFSCLIENT  {self._id}| {url} - {r.status_code}"
                        )
                    ...
                    logger.warning(f"IPFSCLIENT  {self._id}| Retrieved from {url}")
                    self.force_clear_queue_and_stop_task_execution(executor)
                    break
                except Exception as exc:
                    logger.error(
                        f"IPFSClient  {self._id}| {url} generated an exception: {exc}"
                    )
    ...
    def get_metadata_from_gateway(
        ...
        logger.warning(
            f"IPFSCLIENT  {self._id}| get_metadata_from_gateway, \
        ...
        if data is None:
            raise Exception(
                f"IPFSCLIENT  {self._id}| Failed to retrieve CID {multihash} from gateway"
            )
    ...
    def get_metadata_from_ipfs_node(self, multihash, default_metadata_fields):
        logger.warning(
            f"IPFSCLIENT  {self._id}| get_metadata_from_ipfs_node, {multihash}"
        )
        try:
            ...
            if not isinstance(resp_val, dict):
                raise Exception(
                    f"IPFSCLIENT  {self._id}| Expected dict type for {multihash}, received {resp_val}"
                )

        except ValueError as e:
            # Return default format if deserialization fails
            logger.error(
                f"IPFSCLIENT  {self._id}| Failed to deserialize response for {multihash}. {e}"
            )
            raise e
        except Exception as e:
            logger.error(
                f"IPFSCLIENT  {self._id}| Local Node Unknown exception retrieving {multihash}. {e}"
            )
            raise e

        logger.info(f"IPFSCLIENT  {self._id}| Retrieved {multihash} from ipfs node")
    ...
    def cat(self, multihash):
        try:
            ...
            logger.error(
                f"IPFSCLIENT  {self._id}| IPFS cat timed out after 1s for CID {multihash}"
            )
