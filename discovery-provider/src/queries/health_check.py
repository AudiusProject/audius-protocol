...
from src.api_helpers import response_with_signature
...
def version():
    return response_with_signature(disc_prov_version)
...
def health_check():
    ...
    if enforce_block_diff and health_results["block_difference"] > healthy_block_diff:
        return response_with_signature(health_results, 500)

    return response_with_signature(health_results)
