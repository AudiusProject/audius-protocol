...
def get_scorable_playlist_data(session, time_range):
    ...
    # Get all playlists saved within time range:
    ...
    playlists = (
        session.query(Save.save_item_id, Playlist.created_at, Playlist.playlist_owner_id, func.count(Save.save_item_id))
    ...
    playlist_map = {record[0]: {
        ...
        response_name_constants.track_owner_id: record[2],
    ...
    save_counts = get_save_counts(session, False, False, playlist_ids, [RepostType.playlist])
    ...
    for playlist in playlist_map.values():
        owner_id = playlist[response_name_constants.track_owner_id]
...
def make_get_unpopulated_playlists(session, time_range):
    def wrapped():
        ...
        logger.warning(f"{playlist_scoring_data}")
...
def get_trending_playlists(args):
    ...
    with db.scoped_session() as session:
        ...
        # (always requested for `full` endpt
        if with_users:
