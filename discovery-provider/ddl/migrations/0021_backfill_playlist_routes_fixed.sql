...
INSERT INTO playlist_routes (
            ...
            SELECT
                ...
                , REGEXP_REPLACE(CONCAT(REPLACE(REGEXP_REPLACE(LOWER(playlist_name), '[\[\]!"#$%&()*+,./:;<=>''?@\\^_`{|}~]', '', 'g'), ' ', '-'),  '-', playlist_id), '-+', '-', 'g')
                    AS slug
                , REGEXP_REPLACE(CONCAT(REPLACE(REGEXP_REPLACE(LOWER(playlist_name), '[\[\]!"#$%&()*+,./:;<=>''?@\\^_`{|}~]', '', 'g'), ' ', '-'),  '-', playlist_id), '-+', '-', 'g')
                    AS title_slug
                , 0 AS collision_id
...
INSERT INTO playlist_routes (
            ...
            FROM (
                SELECT
                    ...
                    , REGEXP_REPLACE(CONCAT(REPLACE(REGEXP_REPLACE(LOWER(nc.playlist_name), '[\[\]!"#$%&()*+,./:;<=>''?@\\^_`{|}~]', '', 'g'), ' ', '-'),  '-', nc.playlist_id), '-+', '-', 'g') AS slug
                    , REGEXP_REPLACE(CONCAT(REPLACE(REGEXP_REPLACE(LOWER(nc.playlist_name), '[\[\]!"#$%&()*+,./:;<=>''?@\\^_`{|}~]', '', 'g'), ' ', '-'),  '-', nc.playlist_id), '-+', '-', 'g') AS title_slug
                    , 0 AS collision_id
                    , nc.is_current
                    , nc.blockhash
                    , nc.blocknumber
