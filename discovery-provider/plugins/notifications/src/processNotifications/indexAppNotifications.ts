...
export class AppNotificationsProcessor {
  ...
  getIsPushNotificationEnabled(type: string) {
    ...
    const featureEnabled = this.remoteConfig.getFeatureVariableEnabled(MappingFeatureName, mappingVariable)
  ...
  async process(notifications: NotificationRow[]) {
    ...
    for (const notification of mappedNotifications) {
      const isEnabled = this.getIsPushNotificationEnabled(notification.notification.type)
      if (isEnabled) {
        await notification.pushNotification()
      } else {
        logger.info(`Skipping push notification of type ${notification.notification.type}`)
      }
    }
