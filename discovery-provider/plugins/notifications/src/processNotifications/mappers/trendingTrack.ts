...
type TrendingTrackNotificationRow = Omit<NotificationRow, 'data'> & { data: TrendingTrackNotification }
export class TrendingTrack extends BaseNotification<TrendingTrackNotificationRow> {
  ...
  constructor(dnDB: Knex, identityDB: Knex, notification: TrendingTrackNotificationRow) {
    ...
    if ((userNotifications.mobile?.[this.receiverUserId]?.devices ?? []).length > 0) {
      ...
      await Promise.all(devices.map(device => {
        return sendPushNotification({
          ...
          body: `Your Track ${tracks[this.trackId]?.title} is ${this.rank}${rankSuffix} on Trending Right Now! 🍾`,
          data: {}
        })
      }))
  ...
  formatEmailProps(resources: Resources) {
    ...
    return {
      ...
      entity: { type: EntityType.Track, title: track.title, image: track.imageUrl, slug: track.slug }
    }
  }
