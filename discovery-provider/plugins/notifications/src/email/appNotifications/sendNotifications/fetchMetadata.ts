...
const formatGateway = (creatorNodeEndpoint: string) => `${creatorNodeEndpoint.split(',')[0]}/ipfs/`

const getImageUrl = (cid, gateway, defaultImg) =>
  cid ? `${gateway}${cid}` : defaultImg

type Track = { owner_id: number, cover_art_sizes: string, cover_art: string }
type User = { creator_node_endpoint: string, profile_picture_sizes: string, profile_picture: string }
type UserMap = {
  [UserId: number]: User
}

const getUserImage = async (user: User) => {
...
async function getTrackImage(track: Track, userMap: UserMap) {
  const trackOwnerId = track.owner_id
  const trackOwner = userMap[trackOwnerId]
