...
def upgrade():
  ...
  res = session.execute(
    sa.text('''SELECT t."title", t."track_id", t."blockhash", u."user_id", u."handle" FROM "tracks" t INNER JOIN "users" u on "t"."owner_id" = u."user_id" AND u."is_current" = true;''')
  ).fetchall()
  ...
  for i in range(len(route_ids)):
    session.execute(
      sa.text(f"""UPDATE "tracks" SET "route_id" = '{route_ids[i]}' WHERE "blockhash" = '{res[i][2]}' AND "track_id" = '{res[i][1]}'""")
    )
