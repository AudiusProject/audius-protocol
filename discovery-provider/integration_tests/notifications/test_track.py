...
def test_track_create_notification_on_track_update_tons_subscribers(app):
    with app.app_context():
        db = get_db()

    entities = {
        "users": [{"user_id": i + 1} for i in range(2001)],
        "subscriptions": [{"subscriber_id": i, "user_id": 1} for i in range(1, 2000)],
    }
    populate_mock_db(db, entities)
    track_20_creation_datetime = datetime.now()
    entities = {
        "tracks": [
            ...
            {"track_id": 21, "owner_id": 1, "is_playlist_upload": False},
            {"track_id": 2, "owner_id": 2},
        ],
    }
    populate_mock_db(db, entities)
    ...
    with db.scoped_session() as session:
        ...
        assert notification.user_ids == list(range(1, 2000))
