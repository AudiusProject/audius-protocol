...
def test_index_entity_update_and_social_feature(app, mocker):
    ...
    entities = {
        "users": [
            {"user_id": i, "handle": f"user-{i}", "wallet": f"user{i}wallet"} for i in range(1, 13)
        ],
        "playlists": [{"playlist_id": 1, "playlist_owner_id": 10}],
        "reposts": [{"repost_item_id": 1, "repost_type": "playlist", "user_id": i} for i in range(1, 10)],
    }
    populate_mock_db(db, entities)

    test_metadata = {"QmUpdatePlaylist1": {
    ...
    with db.scoped_session() as session:
        # index transactions
        cur_block = session.query(Block).filter(Block.is_current).first()
        print("asdf cur_block", cur_block)
        entity_manager_update(
            None,
            update_task,
            session,
        ...
        indexed_block = session.query(Block).filter(Block.is_current).first()

        print("asdf indexed_block", indexed_block)
        ...
        print("asdf all_notifications ", all_notifications)
