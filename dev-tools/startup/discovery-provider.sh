...
export audius_discprov_url="http://audius-protocol-discovery-${replica}"
...
elasticsearch_host=$(nslookup $(hostname -i) | grep -o "name = .*" | grep -o "[^ ]\+$" | sed 's/discovery/discovery-elasticsearch/')
