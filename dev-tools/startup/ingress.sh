...
# This script:
# - runs a generic nginx reverse proxy
# - updates host's /etc/hosts
...
server {
    $(for i in $PORTS; do echo "listen $i;"; done)
...
IDENTIFIER="audius-protocol-ingress-$RANDOM"
trap "grep -v '\b$IDENTIFIER$' /tmp/hosts > /tmp/hosts.tmp; cat /tmp/hosts.tmp > /tmp/hosts" SIGTERM
while sleep 5; do
  ...
  for cidr in $(split_cidr $(ip addr show eth0 | grep -Eo '(\d+\.){3}\d+/\d+')); do
    ...
    for addr in $alive; do
      ...
      echo "Adding '127.0.0.1 $fqdn $hostname' to /etc/hosts"
      grep -v "\b$fqdn\b" /tmp/hosts > /tmp/hosts.tmp
      echo "127.0.0.1 $fqdn $hostname # $IDENTIFIER" >> /tmp/hosts.tmp
