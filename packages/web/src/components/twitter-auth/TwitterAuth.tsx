    ...
    return window
      ...
      .catch((error) => {
        ...
        reportToSentry({
          ...
          error: 'Twitter getRequestToken popup failed',
          additionalInfo: { error, authenticationUrl, screenName },
          name: 'Sign Up'
        })
      ...
      try {
        if (
          ...
          if (popup.location.search) {
            ...
            reportToSentry({
              level: ErrorLevel.Error,
              error: 'Twitter oauth redirect failed',
  ...
  const getOauthToken = (oAuthVerifier: string, oauthToken: string) => {
    return window
      ...
      .catch((error) => {
        reportToSentry({
          level: ErrorLevel.Error,
          error: 'Twitter getOauthToken failed',
          additionalInfo: { error },
          name: 'Sign Up'
        })
        return onFailure(error)
