...
function* doBuyCryptoViaSol({
  ...
  try {
    ...
    const requiredAmount =
      Number(quoteResponse.otherAmountThreshold) + minRent - existingBalance
    ...
    const transferWSOLInstruction = SystemProgram.transfer({
      ...
      lamports: BigInt(quoteResponse.otherAmountThreshold)
    })
    ...
    if (error) {
      throw new BuyCryptoError(BuyCryptoErrorCode.SWAP_ERROR, error)
    }
