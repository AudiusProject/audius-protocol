...
export const updatePlaylistArtwork = async (
  ...
  const { is_image_autogenerated, cover_art_sizes, artwork } = collection

  // If the playlist has custom artwork (either explicitly marked as not autogenerated
  // or has artwork with a file property indicating user upload), don't override it
  // unless explicitly regenerating
  const hasCustomArtwork =
    !is_image_autogenerated &&
    (cover_art_sizes || (artwork && 'file' in artwork))
  if (hasCustomArtwork && !actions.regenerate) {
    return collection
  }
  ...
  // backfill playlists that don't have cover art
  if (!tracksForImage && !cover_art_sizes) {
    if (tracks.length < 4) {
      tracksForImage = tracks.slice(0, 1)
    } else {
      tracksForImage = tracks.slice(0, 4)
