    ...
    with app.app_context():
        ...
        test_entities = {
            ...
            "aggregate_user": [
                ...
                },  # Should be recommended (similar genre, lower follower count)
                {
                    "user_id": 3,
                    "follower_count": 400,
                    "dominant_genre": "Pop",
                },  # Should not be recommended (too many followers)
                {
                    "user_id": 4,
                    "follower_count": 75,
                    "dominant_genre": "Electronic",
        ...
        assert (
            related[0]["user_id"] == 4
        )  # User 2 should be first (same genre_rank but higher follower count)
        assert (
            related[1]["user_id"] == 2
        )  # User 4 should be second (same genre_rank but lower follower count)
        ...
        assert (
            related[0]["user_id"] == 4
        )  # First related artist should be user 2 (same genre_rank but lower follower count)

        related = get_related_artists(1, None, limit=1, offset=1)
        assert len(related) == 1
        assert (
            related[0]["user_id"] == 2
        )  # Second related artist should be user 4 (same genre_rank but higher follower count)
...
def test_get_related_artists_edge_cases(app):
    ...
    with app.app_context():
        ...
        assert (
            len(related) == 0
        )  # Should get no users since user 1 has no valid tracks to determine genre from
