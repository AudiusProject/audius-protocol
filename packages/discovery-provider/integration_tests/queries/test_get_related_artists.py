...
def test_get_related_artists(app):
    ...
    test_entities = {
        "tracks": [
            ...
            # User 2's tracks - mostly Electronic
        ...
        "follows": [
            {"follower_user_id": 1, "followee_user_id": 3},  # User 1 follows User 3
        ],
        "aggregate_user": [
    ...
    assert len(related) == 2  # Should get user 2 and 4
    assert related[0]["user_id"] == 2  # User 2 should be first (more Electronic tracks)
    assert related[1]["user_id"] == 4  # User 4 should be second

    # Test with filter_followed=True and current_user_id
    related = get_related_artists(1, 1, filter_followed=True)
    assert len(related) == 2  # Should still get users 2 and 4
    
    # Test pagination
    related = get_related_artists(1, None, limit=1)
    assert len(related) == 1
    assert related[0]["user_id"] == 2

    related = get_related_artists(1, None, limit=1, offset=1)
    assert len(related) == 1
    assert related[0]["user_id"] == 4
...
def test_get_related_artists_edge_cases(app):
    ...
    test_entities = {
        "tracks": [
            ...
            # User with deleted/unlisted tracks
            {"track_id": 1, "owner_id": 2, "genre": "Electronic", "is_current": True, "is_delete": True, "is_unlisted": False, "is_available": True},
            {"track_id": 2, "owner_id": 2, "genre": "Electronic", "is_current": True, "is_delete": False, "is_unlisted": True, "is_available": True},
            
            # User with valid tracks
            {"track_id": 3, "owner_id": 3, "genre": "Electronic", "is_current": True, "is_delete": False, "is_unlisted": False, "is_available": True},
        ],
        "users": [
            {"user_id": 1, "handle": "user1"},
            {"user_id": 2, "handle": "user2"},
    ...
    related = get_related_artists(1, None)
    assert len(related) == 1  # Should only get user 3 (user 2's tracks are deleted/unlisted)
    assert related[0]["user_id"] == 3
