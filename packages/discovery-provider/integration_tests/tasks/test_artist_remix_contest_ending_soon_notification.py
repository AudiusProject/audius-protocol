...
def test_artist_remix_contest_ending_soon_notification_private_track(app):
    """Test that no notification is created for a remix contest on a private (unlisted) track for the artist"""
    with app.app_context():
        db = get_db()
    ...
    with db.scoped_session() as session:
        ...
        # Should not notify the artist for private tracks
