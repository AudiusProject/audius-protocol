...
begin
  ...
  if new.event_type = 'remix_contest' and new.is_deleted = false then
    -- Get the owner of the track that triggered the contest
    select owner_id into owner_user_id from tracks where is_current and track_id = new.entity_id limit 1;
