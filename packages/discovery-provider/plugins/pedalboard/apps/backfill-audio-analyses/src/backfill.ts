...
async function analyzeAudio(contentNodes: string[], track: Track): Promise<void> {
  ...
  for (const contentNodeEndpoint of contentNodes) {
    try {
      ...
      const response = await axios.post(`${analysisUrl}?${queryParams.toString()}`, null, { timeout: REQUEST_TIMEOUT });
      if (response.status !== 200) {
        console.log(`Received ${response.status} response from ${contentNodeEndpoint}. Trying another content node...`)
        continue
      }
    } catch (error: any) {
      if (error.isAxiosError !== undefined && error.code === 'ECONNABORTED') {
        console.log(`Timeout error triggering audio analysis for track ID ${track.track_id} on ${contentNodeEndpoint}: ${error.message}. Trying another content node...`);
        continue
      } else {
        console.error(`Error analyzing audio for track ID ${track.track_id}, upload ID ${track.audio_upload_id}: ${(error as Error).message}. Skipping track...`);
        break
      }
    }
...
async function fetchTracks(offset: number, limit: number, db: Knex): Promise<Track[]> {
...
async function processBatches(db: Knex): Promise<void> {
  ...
  while (true) {
    ...
    const endpoints = _.shuffle(contentNodes.map(node => node.endpoint)).slice(0, 5)
    ...
    console.timeEnd('Batch processing time');
    console.log(`Triggered audio analyses for ${tracks.length} tracks. New offset: ${offset}`);

    if (config.testRun) {
      console.log(`[TEST RUN] Triggered audio analyses for the following track IDs: ${tracks.map(track => track.track_id)}`)
