...
from src.tasks.entity_manager.utils import EntityType, ManageEntityParameters, copy_record, validate_signer
...
def validate_update_event_tx(params: ManageEntityParameters):
    ...
    existing_event = params.existing_records.get(EntityType.EVENT, {}).get(params.entity_id)
    ...
    if user_id != existing_event.user_id:
        raise IndexingValidationError(
            f"Only event owner can update event {event_id}"
        )


def update_event(params: ManageEntityParameters):
    validate_update_event_tx(params)
    event_record = params.existing_records.get(EntityType.EVENT, {}).get(params.entity_id)
    ...
    event_record.entity_type = params.metadata.get('entity_type', event_record.entity_type)
...
def validate_delete_event_tx(params: ManageEntityParameters):
    ...
    if user_id != existing_event.user_id:
        raise IndexingValidationError(
            f"Only event owner can delete event {event_id}"
        )
...
def delete_event(params: ManageEntityParameters):
    ...
    if params.entity_id in params.new_records[EntityType.EVENT.value]:
        existing_event = params.new_records[EntityType.EVENT.value][
            params.entity_id
        ][-1]
