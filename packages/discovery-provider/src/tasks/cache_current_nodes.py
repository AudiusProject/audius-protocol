...
def cache_current_nodes_task(self):
    ...
    try:
        ...
        if have_lock:
            logger.debug("cache_current_nodes.py | fetching all other discovery nodes")
            (
            ...
            logger.debug(
                "cache_current_nodes.py | set current discovery nodes in redis"
            )

            logger.debug("cache_current_nodes.py | fetching all other content nodes")
            ...
            logger.debug("cache_current_nodes.py | set current content nodes in redis")

            logger.debug("cache_current_nodes.py | checking content nodes for liveness")
            ...
            logger.debug("cache_current_nodes.py | set alive content nodes in redis")
        else:
            logger.debug("cache_current_nodes.py | Failed to acquire lock")
