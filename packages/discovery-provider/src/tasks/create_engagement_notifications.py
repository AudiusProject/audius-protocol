...
@log_duration(logger)
def create_claimable_reward_notifications(session):
...
@log_duration(logger)
def download_app_notifications(session):
    week_cooldown_interval = timedelta(weeks=1)
    ...
    try:
        ...
        if have_lock:
            with db.scoped_session() as session:
                create_claimable_reward_notifications(session)
                create_download_app_notifications(session)
