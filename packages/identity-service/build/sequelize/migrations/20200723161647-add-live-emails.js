...
module.exports = {
    up: (queryInterface, Sequelize) => {
        ...
        return queryInterface.sequelize.query("ALTER TYPE \"enum_NotificationEmails_emailFrequency\" ADD VALUE 'live'")
            ...
            .finally(() => {
            return queryInterface.sequelize.query("ALTER TABLE \"NotificationEmails\" ALTER \"emailFrequency\" set default 'live'::\"enum_NotificationEmails_emailFrequency\"");
        })
            // Add 'live' to "UserNotificationSettings" "emailFrequency"
            .then(() => {
            return queryInterface.sequelize.query("ALTER TYPE \"enum_UserNotificationSettings_emailFrequency\" ADD VALUE 'live'");
        })
            // Do nothing if it already exists
            // Deleting enum values is not possible w/o root db access, so in the case the enum already exists
            // just continue on. If for some reason this threw but it didn't exist, we would fail at the next step
            ...
            .finally(() => {
            return queryInterface.sequelize.query("ALTER TABLE \"UserNotificationSettings\" ALTER \"emailFrequency\" set default 'live'::\"enum_UserNotificationSettings_emailFrequency\"");
        })
    ...
    down: (queryInterface, Sequelize) => {
        ...
        return models.UserNotificationSettings.update({
            ...
            .then(() => {
            return queryInterface.sequelize.query("ALTER TABLE \"NotificationEmails\" ALTER \"emailFrequency\" set default 'daily'::\"enum_NotificationEmails_emailFrequency\"");
        })
            ...
            // Change the default back to 'daily'
            .then(() => {
            return queryInterface.sequelize.query("ALTER TABLE \"UserNotificationSettings\" ALTER \"emailFrequency\" set default 'daily'::\"enum_UserNotificationSettings_emailFrequency\"");
        });
        // Normal users can't do this!
        // Delete 'live' from "NotificationEmails" "emailFrequency"
        // .then(() => {
