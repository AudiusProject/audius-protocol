...
async function processCosignNotifications(notifications, tx) {
    ...
    for (const notification of notifications) {
        const { entity_id: childTrackId, entity_owner_id: childTrackUserId } = notification.metadata;
        ...
        const cosignNotifications = await models.Notification.findAll({
            where: {
        ...
        if (cosignNotifications.length > 0)
            continue;
        ...
        const cosignNotification = await models.Notification.create({
            type: notificationTypes.RemixCosign,
            userId: childTrackUserId,
            entityId: childTrackId,
            blocknumber,
