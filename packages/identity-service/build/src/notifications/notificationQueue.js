...
const { getRemoteFeatureVarEnabled, DISCOVERY_NOTIFICATION_MAPPING, MappingVariable } = require('../remoteConfig');
 ...
 */
async function promiseAllInBatches(task, optimizelyClient, logger, items, batchSize) {
    let position = 0;
    let results = [];
    while (position < items.length) {
        ...
        results = [
            ...results,
            ...(await Promise.allSettled(itemsForBatch.map((item) => task(optimizelyClient, logger, item))))
        ];
        position += batchSize;
    }
    return results;
...
const notificaitonTypeMapping = {
    ...
    [notificationTypes.SupporterDethroned]: MappingVariable.PushSupporterDethroned,
...
async function processNotification(optimizelyClient, logger, notification) {
    ...
    const notificationMappingVar = notificaitonTypeMapping[notification.notification.type];
    ...
    const isDisabled = getRemoteFeatureVarEnabled(optimizelyClient, DISCOVERY_NOTIFICATION_MAPPING, notificationMappingVar);
    if (isDisabled === true) {
        return;
    }
...
async function drainPublishedMessages(logger, optimizelyClient) {
    ...
    const numProcessedNotifications = await promiseAllInBatches(processNotification, optimizelyClient, logger, pushNotificationQueue.PUSH_NOTIFICATIONS_BUFFER, BATCH_SIZE);
...
async function drainPublishedSolanaMessages(logger, optimizelyClient) {
    ...
    for (const bufferObj of pushNotificationQueue.PUSH_SOLANA_NOTIFICATIONS_BUFFER) {
        const notificationMappingVar = notificaitonTypeMapping[bufferObj.notification.type];
        // NOTE: This flag is used betwen the notifications plugin and identity service
        // so, when true, it is enabled in the plugin and disabeld here in identity
        const isDisabled = getRemoteFeatureVarEnabled(optimizelyClient, DISCOVERY_NOTIFICATION_MAPPING, notificationMappingVar);
        if (isDisabled === true) {
