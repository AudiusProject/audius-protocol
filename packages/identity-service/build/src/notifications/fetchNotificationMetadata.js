...
async function getEmailNotifications(audius, userId, announcements = [], fromTime = getLastWeek(), limit = 5) {
    try {
        ...
        const validUserAnnouncements = announcements.filter((a) => moment(a.datePublished).isAfter(user.createdAt) &&
...
isEmailNotif = false) {
    ...
    for (const notification of notifications) {
        switch (notification.type) {
            ...
            case NotificationType.TierChange: {
                userIdsToFetch.push(...notification.actions
    ...
    userIdsToFetch.push(...tracks.map(({ owner_id: id }) => id), ...collections.map(({ playlist_owner_id: id }) => id));
...
const formatGateway = (creatorNodeEndpoint) => creatorNodeEndpoint
    ? `${creatorNodeEndpoint.split(',')[0]}/ipfs/`
    : USER_NODE_IPFS_GATEWAY;
const getImageUrl = (cid, gateway, defaultImg) => cid ? `${gateway}${cid}` : defaultImg;
