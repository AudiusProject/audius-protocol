...
const { getRemoteFeatureVarEnabled, NOTIFICATIONS_EMAIL_PLUGIN, EmailPluginMappings } = require('../remoteConfig');
// Sendgrid object
let sg;
...
async function processEmailNotifications(expressApp, audiusLibs) {
    try {
        ...
        const isLiveEmailDisabled = getRemoteFeatureVarEnabled(optimizelyClient, NOTIFICATIONS_EMAIL_PLUGIN, EmailPluginMappings.Live);
        const isScheduledEmailDisabled = getRemoteFeatureVarEnabled(optimizelyClient, NOTIFICATIONS_EMAIL_PLUGIN, EmailPluginMappings.Scheduled);
        let liveEmailUsers = [];
        let dailyEmailUsers = [];
        ...
        liveUsersWithPendingAnnouncements.forEach((item) => pendingNotificationUsers.add(item));
        dailyUsersWithPendingAnnouncements.forEach((item) => pendingNotificationUsers.add(item));
        weeklyUsersWithPendingAnnouncements.forEach((item) => pendingNotificationUsers.add(item));
        ...
        logger.info(loggingContext, `processEmailNotifications - Live Email Users: ${liveEmailUsersWithUnseenNotifications}, Daily Email Users: ${dailyEmailUsersWithUnseeenNotifications}, Weekly Email Users: ${weeklyEmailUsersWithUnseeenNotifications}`);
        ...
        for (let chunk = 0; chunk * chuckSize < userInfo.length; chunk += 1) {
            ...
            const chunkResults = await Promise.all(userInfo.slice(start, end).map(async (user) => {
                try {
                    let { email: userEmail, blockchainUserId: userId, timezone, isBlockedFromEmails } = user;
                    ...
                    const sent = await renderAndSendNotificationEmail(userId, userEmail, appAnnouncements, frequency, startTime, audiusLibs);
                    if (!sent) {
...
async function renderAndSendNotificationEmail(userId, userEmail, announcements, frequency, startTime, audiusLibs) {
    try {
        logger.debug(`renderAndSendNotificationEmail | ${userId}, ${userEmail}, ${frequency}, from ${startTime}`);
        const timeBeforeEmailNotifications = Date.now();
        const [notificationProps, notificationCount] = await getEmailNotifications(audiusLibs, userId, announcements, startTime, 5);
        ...
        logger.debug(`renderAndSendNotificationEmail | time after getEmailNotifications | ${timeAfterEmailNotifications} | time elapsed is ${getEmailDuration} | ${notificationCount} unread notifications`);
        const emailSubject = `${notificationCount} unread notification${notificationCount > 1 ? 's' : ''} on Audius`;
        if (notificationCount === 0) {
            logger.debug(`renderAndSendNotificationEmail | 0 notifications detected for user ${userId}, bypassing email`);
            return;
        }
        const renderProps = {
        ...
        const weeklySubjectFormat = `${notificationCount} unread notification${notificationCount > 1 ? 's' : ''} from ${shortWeekAgoFormat} - ${formattedDayAgo}`;
        const dailySubjectFormat = `${notificationCount} unread notification${notificationCount > 1 ? 's' : ''} from ${formattedDayAgo}`;
        let subject;
        if (frequency === EmailFrequency.LIVE) {
