...
const { updateBlockchainIds, getHighestSlot, getHighestBlockNumber
...
const { notificationJobType, solanaNotificationJobType, announcementJobType, unreadEmailJobType, downloadEmailJobType } = require('./constants');
...
class NotificationProcessor {
    constructor({ errorHandler }) {
        ...
        this.solanaNotifQueue = new Bull(`solana-notification-queue-${Date.now()}`, {
            redis: {
                port: config.get('redisPort'),
                host: config.get('redisHost')
            },
    ...
    async init(audiusLibs, expressApp, redis) {
        ...
        this.solanaNotifQueue.process(async (job) => {
            ...
            try {
                if (!minSlot && minSlot !== 0)
                    throw new Error('no min slot');
                const oldMaxSlot = await this.redis.get('maxSlot');
                let maxSlot = null;
                // Index notifications
                if (minSlot < oldMaxSlot) {
                    logger.error('solana notification queue processing error - tried to process a minSlot < oldMaxSlot', minSlot, oldMaxSlot);
                    maxSlot = oldMaxSlot;
                }
                else {
                    const optimizelyClient = expressApp.get('optimizelyClient');
                    ...
                    if (maxSlot < minSlot) {
                        logger.error('solana notification queue processing error - unexpectedly got maxSlot < minSlot from Discovery, using old minSlot as max', minSlot, oldMaxSlot);
                        maxSlot = minSlot;
                    }
                }
        ...
        const startBlock = await getHighestBlockNumber();
    ...
    async indexAll(audiusLibs, optimizelyClient, minBlock, oldMaxBlockNumber) {
        ...
        let notifications = await filterOutAbusiveUsers(notificationsFromDN.notifications);
        ...
        try {
            ...
            const numProcessedNotifs = await drainPublishedMessages(logger, optimizelyClient);
    ...
    async indexAllSolanaNotifications(audiusLibs, optimizelyClient, minSlot, oldMaxSlot) {
        ...
        try {
            ...
            const processedNotifications = await processNotifications(notifications, tx, optimizelyClient);
            ...
            time = Date.now();
            // actually send out push notifications
            const numProcessedNotifs = await drainPublishedSolanaMessages(logger, optimizelyClient);
...
async function filterOutAbusiveUsers(notifications) {
    ...
    const result = notifications.filter((notification) => {
        ...
        const isUserEntityAbusive = notification.metadata &&
