...
const getPublishUserId = (notif, baseType) => {
    if (baseType === notificationTypes.Follow)
        ...
        return notif.metadata.entity_owner_id;
    else if (baseType === notificationTypes.Create.base)
        return notif.subscriberId;
    else if (baseType === notificationTypes.ChallengeReward)
        return notif.initiator;
    else if (baseType === notificationTypes.Milestone)
        return notif.initiator;
    else if (baseType === notificationTypes.TierChange)
        return notif.initiator;
    ...
    else if (baseType === notificationTypes.TipReceive)
        return notif.initiator;
...
const publishNotifications = async (notifications, metadata, userNotificationSettings, tx, optimizelyClient) => {
    ...
    for (const notification of notifications) {
        ...
        const isInitiatorAbusive = initiatorMap[initiatorUserId] &&
        ...
        if (solanaNotificationBaseTypes.includes(notification.type)) {
            await publishSolanaNotification(msg, userId, tx, true, title, types, notification);
        }
        else {
