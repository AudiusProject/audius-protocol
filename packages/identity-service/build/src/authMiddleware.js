...
async function authMiddleware(req, res, next) {
    try {
        ...
        if (!encodedDataMessage)
            throw new Error('[Error]: Encoded data missing');
        if (!signature)
            throw new Error('[Error]: Encoded data signature missing');
        ...
        if (!user)
            throw new Error(`[Error]: no user found for wallet address ${walletAddress}`);
        ...
        const errorResponse = errorResponseBadRequest(`[Error]: The wallet address is not associated with a user id ${err}`);
