...
if (vapidKeys.publicKey && vapidKeys.privateKey && browserPushGCMAPIKey) {
    ...
    webpush.setVapidDetails('mailto:contact@audius.co', vapidKeys.publicKey, vapidKeys.privateKey);
...
const sendBrowserNotification = async ({ userId, notificationParams }) => {
    ...
    try {
        ...
        const notificationBrowsers = await models.NotificationBrowserSubscription.findAll({
...
const apnAuthKeyPath = path.resolve(__dirname, './notifications/browserPush/audius.pushpackage/AuthKey.p8');
if (apnAuthKey) {
