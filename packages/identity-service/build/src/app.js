...
const { getRateLimiter, getRateLimiterMiddleware, getRelayBlocklistMiddleware, getRelayRateLimiterMiddleware, isIPWhitelisted, getIP } = require('./rateLimiter.js');
...
class App {
    ...
    async init() {
        ...
        if (cluster.isMaster) {
            ...
            if (process.env.WORKER_TYPE === 'notifications') {
                await this.notificationProcessor.init(audiusInstance, this.express, this.redisClient);
            }
            else {
                await new Promise((resolve) => {
                    server = this.express.listen(this.port, resolve);
    ...
    _createRateLimitsForListenCounts(interval, timeInSeconds) {
        const listenCountLimiter = getRateLimiter({
            ...
            max: config.get(`rateLimitingListensPerTrackPer${interval}`),
            skip: function (req) {
        ...
        const listenCountIPTrackLimiter = getRateLimiter({
            ...
            expiry: timeInSeconds,
            max: config.get(`rateLimitingListensPerIPTrackPer${interval}`),
            keyGenerator: function (req) {
                const trackId = req.params.id;
                const { ip } = getIP(req);
                return `${ip}:::${trackId}`;
        ...
        const listenCountIPRequestLimiter = getRateLimiter({
            ...
            expiry: timeInSeconds,
            max: config.get(`rateLimitingListensPerIPPer${interval}`),
            keyGenerator: function (req) {
                const { ip } = getIP(req);
                return `${ip}`;
            }
    ...
    setRateLimiters() {
        ...
        // This limiter double dips with the reqLimiter. The 5 requests every hour are also counted here
        this.express.use('/tracks/:id/listen', listenCountWeeklyIPTrackLimiter, listenCountWeeklyLimiter, listenCountDailyIPTrackLimiter, listenCountDailyLimiter, listenCountHourlyIPTrackLimiter, listenCountHourlyLimiter, listenCountDailyIPLimiter);
        ...
        this.express.use('/eth_relay', ethRelayWalletRateLimiter, ethRelayIPRateLimiter);
        this.express.use('/relay', getRelayBlocklistMiddleware, getRelayRateLimiterMiddleware());
