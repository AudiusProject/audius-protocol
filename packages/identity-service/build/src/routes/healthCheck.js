...
module.exports = function (app) {
    ...
    */
    app.get('/health_check/relay', handleResponse(async (req, res) => {
        ...
        await redis.zremrangebyscore('relayTxAttempts', '-inf', epochTenMinutesAgo);
        await redis.zremrangebyscore('relayTxFailures', '-inf', epochTenMinutesAgo);
        await redis.zremrangebyscore('relayTxSuccesses', '-inf', epochTenMinutesAgo);
        // check if there have been any attempts in the time window that we processed the block health check
        const attemptedTxsInRedis = await redis.zrange('relayTxAttempts', '0', '-1');
        const successfulTxsInRedis = await redis.zrange('relayTxSuccesses', '0', '-1');
