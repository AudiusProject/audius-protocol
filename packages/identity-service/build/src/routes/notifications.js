...
const formatNotifications = (notifications, announcements) => {
    ...
    const userNotifications = notifications
        ...
        if (mapResponse)
            return mapResponse(notification, userAnnouncements);
    })
        .filter(Boolean);
...
module.exports = function (app) {
    ...
    app.get('/notifications', authMiddleware, handleResponse(async (req) => {
        ...
        if (isNaN(limit) || limit > 100) {
            return errorResponseBadRequest(`Limit and offset number be integers with a max limit of 100`);
        }
        try {
            ...
            const unViewedSolanaCount = await models.SolanaNotification.findAll({
                ...
                attributes: [
                    [
                        models.Sequelize.fn('COUNT', models.Sequelize.col('SolanaNotification.id')),
                        'total'
                    ]
                ],
            ...
            const validUserAnnouncements = announcements.filter((a) => moment(a.datePublished).isAfter(createdDate));
            const announcementsAfterFilter = validUserAnnouncements.filter((a) => timeOffset.isAfter(moment(a.datePublished)));
            const unreadAnnouncementCount = validUserAnnouncements.length - viewedAnnouncementCount;
            const userNotifications = formatNotifications(notifications
                .concat(solanaNotifications)
                .concat(filteredViewedAnnouncements), announcementsAfterFilter);
            let playlistUpdates = [];
            if (walletAddress) {
                ...
                if (playlistUpdatesResult) {
                    ...
                    playlistUpdates = Object.keys(playlistUpdatesResult)
                        .filter((playlistId) => playlistUpdatesResult[playlistId].userLastViewed >=
                        thirtyDaysAgo &&
    ...
    app.post('/notifications/all', authMiddleware, handleResponse(async (req, res, next) => {
        ...
        try {
            ...
            await models.Notification.bulkCreate(unreadAnnouncements.map((announcement) => ({
                type: NotificationType.Announcement,
                entityId: announcement.entityId,
                ...update,
                isHidden: false,
