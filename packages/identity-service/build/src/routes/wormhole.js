 ...
 */
const relayWormhole = async (req, audiusLibs, senderAddress, permit, transferTokens, reportError) => {
    const logs = [];
    ...
    try {
        ...
        let numRetries = 0;
        const MAX_RETRIES = 5;
        while ((!response || !response.transactionSignature) &&
            numRetries < MAX_RETRIES) {
            // Gather VAA attestations, submit to solana and realize the funds at the taret address
            response =
                await audiusLibs.wormholeClient.attestAndCompleteTransferEthToSol(transferTxHash, signTransaction, {
    ...
    app.post('/wormhole_relay', async (req, res, next) => {
        ...
        try {
            ...
            if (!body.senderAddress ||
                ...
                return sendResponse(req, res, errorResponseBadRequest('Missing one of the required fields: senderAddress, permit, transferTokens'));
            }
            const { transferTxHash, redeemTxSignature, error } = await relayWormhole(req, audiusLibs, body.senderAddress, body.permit, body.transferTokens, reportError);
            if (error) {
            ...
            return sendResponse(req, res, errorResponseServerError(error.message.toString()));
        }
    });
};
