...
module.exports = function (app) {
    app.post('/relay', captchaMiddleware, handleResponse(async (req, res, next) => {
        ...
        try {
            ...
            detectAbuseOnRelay =
                getFeatureFlag(optimizelyClient, FEATURE_FLAGS.DETECT_ABUSE_ON_RELAY) && !req.isFromApp;
            blockAbuseOnRelay =
                getFeatureFlag(optimizelyClient, FEATURE_FLAGS.BLOCK_ABUSE_ON_RELAY) && !req.isFromApp;
        }
        catch (error) {
        ...
        const userFlaggedAsAbusive = user &&
        ...
        if (body &&
            ...
            (async () => {
                try {
                    ...
                    req.logger.error(`Error setting provisional handle for user ${user.wallet}: ${e.message}`);
                }
            })();
            // send tx
            let receipt;
            ...
            try {
                ...
                receipt = await txRelay.sendTransaction(req, false, // resetNonce
                txProps, reqBodySHA);
            }
            catch (e) {
                if (e.message.includes('nonce')) {
                    ...
                    receipt = await txRelay.sendTransaction(req, true, // resetNonce
                    txProps, reqBodySHA);
                    ...
                    return errorResponseServerError(`Something caused the transaction to fail for payload ${reqBodySHA}, ${e.message}`);
                }
            }
