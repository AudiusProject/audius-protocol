...
module.exports = function (app) {
    ...
    app.post('/tracks/:id/listen', handleResponse(async (req, res) => {
        ...
        req.logger.info(`TrackListen userId=${userId} ip=${ip} isWhitelisted=${isWhitelisted}`);
        ...
        req.logger.info(`TrackListen tx submission, forwardedIP=${ip} trackId=${trackId} userId=${userId}, ${JSON.stringify(trackingRedisKeys)}`);
        await redis.incr(trackingRedisKeys.submission);
        await redis.zadd(TRACKING_LISTEN_SUBMISSION_KEY, Date.now(), Date.now() + entropy);
        let location;
        try {
            const url = `https://api.ipdata.co/${ip}?api-key=${config.get('ipdataAPIKey')}`;
