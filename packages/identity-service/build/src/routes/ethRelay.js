...
module.exports = function (app) {
    ...
    app.post('/eth_relay', async (req, res, next) => {
        ...
        if (body && body.contractAddress && body.senderAddress && body.encodedABI) {
            ...
            try {
                ...
                return sendResponse(req, res, errorResponseServerError(`Something caused the transaction to fail for payload ${reqBodySHA}, ${e.message}`));
            }
        }
        else {
            return sendResponse(req, res, errorResponseServerError('Missing one of the required fields: contractRegistryKey, contractAddress, senderAddress, encodedABI'));
        }
    });
    ...
    app.get('/eth_relayer', handleResponse(async (req, res, next) => {
        ...
        if (!wallet)
            return errorResponseBadRequest('Please provide a wallet');
    ...
    app.get('/eth_gas_price', handleResponse(async (req, res, next) => {
        const gasInfo = await ethTxRelay.getProdGasInfo(req.app.get('redis'), req.logger);
