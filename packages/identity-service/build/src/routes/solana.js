...
const isValidInstruction = (instr) => {
    if (!instr || !Array.isArray(instr.keys) || !instr.programId || !instr.data)
        return false;
    if (!instr.keys.every((key) => !!key.pubkey))
        return false;
    return true;
};
solanaRouter.post('/relay', parameterizedAuthMiddleware({ shouldResponseBadRequest: false }), handleResponse(async (req, res, next) => {
    ...
    const isRelayAllowed = await areRelayAllowedInstructions(instructions, optimizelyClient);
    if (!isRelayAllowed) {
    ...
    if (socialProofRequiredToSend && isSendInstruction(instructions)) {
        ...
        if (!userHasSocialProof) {
            ...
            return errorResponseServerError(`User ${handle} is missing social proof`, { error: 'Missing social proof' });
        }
    }
