"use strict";
const { handleResponse, successResponse, errorResponse, errorResponseBadRequest } = require('../apiHelpers');
...
module.exports = function (app) {
    app.post('/stripe/session', handleResponse(async (req) => {
        ...
        if (destinationCurrency !== 'sol' && destinationCurrency !== 'usdc') {
            return errorResponseBadRequest('Invalid destination currency: only support sol and usdc');
        }
        ...
        urlEncodedData.append('transaction_details[supported_destination_currencies[]', 'usdc');
        try {
            const req = {
            ...
            return errorResponse(e.response.status, 'Failed to create Stripe session');
        }
    }));
};
