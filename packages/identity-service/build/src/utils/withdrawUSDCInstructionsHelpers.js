...
const checkCreateAccountInstruction = (instruction) => {
    return (instruction.programId === ASSOCIATED_TOKEN_PROGRAM_ID.toString() &&
...
const checkCloseAccountInstruction = (instruction) => {
    ...
    const feePayerPubkeys = feePayerKeypairs.map((wallet) => Keypair.fromSecretKey(Uint8Array.from(wallet.privateKey)).publicKey.toString());
    const isCloseInstruction = instruction.programId === TOKEN_PROGRAM_ID.toString() &&
        getInstructionEnum(instruction) === CLOSE_ACCOUNT_INSTRUCTION;
    const isFeePayerReimbursed = feePayerPubkeys.includes(instruction.keys[FEE_PAYER_ACCOUNT_INDEX].pubkey);
...
const checkJupiterSwapInstructions = (instructions) => {
    const createAccountInstruction = instructions[CREATE_TOKEN_ACCOUNT_INSTRUCTION_INDEX];
    const associatedTokenAccountOwner = createAccountInstruction.keys[CREATE_TOKEN_ACCOUNT_OWNER_INDEX];
    const associatedTokenAccount = createAccountInstruction.keys[CREATE_TOKEN_ACCOUNT_ASSOCIATED_TOKEN_ACCOUNT_INDEX];
    // Check that the create associated token account instruction is correct
    const createAssociatedTokenAccountInstruction = instructions[JUPITER_CREATE_ASSOCIATED_TOKEN_ACCOUNT_INSTRUCTION_INDEX];
    if (createAssociatedTokenAccountInstruction.programId !==
        ASSOCIATED_TOKEN_PROGRAM_ID.toString()) {
        return false;
    }
    const feePayerKey = createAssociatedTokenAccountInstruction.keys[JUPITER_CREATE_ASSOCIATED_TOKEN_ACCOUNT_FEE_PAYER_ACCOUNT_INDEX];
    const tempHoldingAccount = createAssociatedTokenAccountInstruction.keys[JUPITER_CREATE_ASSOCIATED_TOKEN_ACCOUNT_TEMP_HOLDING_ACCOUNT_INDEX];
    const tempHoldingAccountOwner = createAssociatedTokenAccountInstruction.keys[JUPITER_CREATE_ASSOCIATED_TOKEN_ACCOUNT_TEMP_HOLDING_ACCOUNT_OWNER_INDEX];
    ...
    const setTokenLedgerInstruction = instructions[JUPITER_SET_TOKEN_LEDGER_INSTRUCTION_INDEX];
    if (setTokenLedgerInstruction.programId !== JUPITER_AGGREGATOR_V3_PROGRAM_ID) {
        return false;
    }
    const isCorrectTokenLedgerTokenAccount = setTokenLedgerInstruction.keys[JUPITER_SET_TOKEN_LEDGER_TOKEN_ACCOUNT_INDEX]
    ...
    const isCorrectHoldingAccountOwner = swapInstruction.keys[JUPITER_SWAP_TEMP_HOLDING_ACCOUNT_OWNER_INDEX]
        .pubkey === tempHoldingAccountOwner.pubkey;
    const isCorrectHoldingAccount = swapInstruction.keys[JUPITER_SWAP_TEMP_HOLDING_ACCOUNT_INDEX].pubkey ===
        tempHoldingAccount.pubkey;
    const isCorrectRecipient = swapInstruction.keys[JUPITER_SWAP_RECIPIENT_ACCOUNT_INDEX].pubkey ===
        associatedTokenAccount.pubkey;
    if (!isCorrectHoldingAccountOwner ||
        !isCorrectHoldingAccount ||
        !isCorrectRecipient) {
        return false;
    }
    // Check that the close associated token account instruction is correct
    const closeAssociatedTokenAccountInstruction = instructions[JUPITER_CLOSE_ASSOCIATED_TOKEN_ACCOUNT_INSTRUCTION_INDEX];
    if (closeAssociatedTokenAccountInstruction.programId !==
        TOKEN_PROGRAM_ID.toString()) {
        return false;
    }
    const isCorrectClosedAccount = closeAssociatedTokenAccountInstruction.keys[JUPITER_CLOSE_ASSOCIATED_TOKEN_ACCOUNT_TEMP_HOLDING_ACCOUNT_INDEX].pubkey === tempHoldingAccount.pubkey;
    const isCorrectClosedAccountDestination = closeAssociatedTokenAccountInstruction.keys[JUPITER_CLOSE_ASSOCIATED_TOKEN_ACCOUNT_DESTINATION_INDEX].pubkey === tempHoldingAccountOwner.pubkey;
    const isCorrectClosedAccountOwner = closeAssociatedTokenAccountInstruction.keys[JUPITER_CLOSE_ASSOCIATED_TOKEN_ACCOUNT_TEMP_HOLDING_ACCOUNT_OWNER_INDEX].pubkey === tempHoldingAccountOwner.pubkey;
    if (!isCorrectClosedAccount ||
...
const checkCreateAccountMatchesClose = (instructions) => {
    ...
    return (createAccountInstruction.keys[ACCOUNT_TO_CREATE_INDEX].pubkey ===
...
const isUSDCWithdrawalTransaction = (instructions) => {
    if (!instructions.length)
        return false;
    const validations = [];
    validations.push(checkCreateAccountInstruction(instructions[CREATE_TOKEN_ACCOUNT_INSTRUCTION_INDEX]));
    const transferInstructions = instructions.slice(USER_BANK_TRANSFER_INDEX_START, USER_BANK_TRANSFER_INDEX_END);
    ...
    validations.push(checkCloseAccountInstruction(instructions[instructions.length - 1]));
