...
const { isUSDCWithdrawalTransaction } = require('./withdrawUSDCInstructionsHelpers');
const { getFeatureFlag, FEATURE_FLAGS } = require('../featureFlag');
const { solanaClaimableTokenProgramAddress, solanaMintAddress, solanaRewardsManager, solanaRewardsManagerProgramId, rewardManagerBaseAccountIndices, usdcMintAddress, claimableTokenAuthorityIndices, isRelayAllowedProgram, getInstructionEnum } = require('./relayUtils');
...
const deriveClaimableTokenAuthority = async (mint) => {
    return (await SolanaUtils.findProgramAddressFromPubkey(SolanaUtils.newPublicKeyNullable(solanaClaimableTokenProgramAddress), SolanaUtils.newPublicKeyNullable(mint)))[0].toString();
};
...
const isRelayAllowedInstruction = async (instruction) => {
    if (instruction.programId === solanaRewardsManagerProgramId) {
        ...
        return checkAccountKey(instruction, getAccountIndex(instruction, rewardManagerBaseAccountIndices), solanaRewardsManager);
    }
        ...
        return (checkAccountKey(instruction, getAccountIndex(instruction, claimableTokenAuthorityIndices), audioAuthority) ||
            checkAccountKey(instruction, getAccountIndex(instruction, claimableTokenAuthorityIndices), usdcAuthority));
    }
...
const areRelayAllowedInstructions = async (instructions, optimizelyClient) => {
    const results = await Promise.all(instructions.map((instruction) => isRelayAllowedInstruction(instruction)));
