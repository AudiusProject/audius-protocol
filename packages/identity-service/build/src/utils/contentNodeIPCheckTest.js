...
describe('test contentNodeIPCheck', () => {
    it('returns true if already known', () => {
        const isIPFromContentNode = _isIPFromContentNode('127.0.0.1', {
            body: {
    ...
    it('returns true if signature produces a valid known wallet', () => {
        const { signature, timestamp } = apiSigning.generateTimestampAndSignature({ data: 'listen' }, '0x76cade66d94d4bcb9352efee88854cc511cd918cb4bf29e4a5375900b5ef0e3c');
        const isIPFromContentNode = _isIPFromContentNode('127.0.0.1', {
            body: {
                signature,
                timestamp
        ...
        new Set(['0x209220b86C13EED39c126E1824adb7Cb5F548835']), new Set([]));
        assert.strictEqual(isIPFromContentNode, true);
    });
});
