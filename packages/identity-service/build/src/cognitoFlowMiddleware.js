...
async function cognitoFlowMiddleware(req, res, next) {
    ...
    if (!isWebhookValid(headers, url)) {
        const errorResponse = errorResponseBadRequest('[Error]: The cognito flow webhook is invalid.');
        return sendResponse(req, res, errorResponse);
    }
    const timeDifferenceMilliseconds = Date.now() - new Date(headers.date).getTime();
    if (timeDifferenceMilliseconds > MAX_TIME_DRIFT_MILLISECONDS) {
        const errorResponse = errorResponseBadRequest('[Error]: The cognito flow webhook timestamp is too old.');
        return sendResponse(req, res, errorResponse);
    }
    next();
}
