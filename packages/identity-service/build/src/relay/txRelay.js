...
const UPDATE_REPLICA_SET_RECONFIGURATION_LIMIT = config.get('updateReplicaSetReconfigurationLimit');
const UPDATE_REPLICA_SET_WALLET_WHITELIST = config.get('updateReplicaSetWalletWhitelist');
...
const sendTransactionInternal = async (req, web3, txProps, reqBodySHA) => {
    let { contractRegistryKey, contractAddress, nethermindContractAddress, encodedABI, nethermindEncodedABI, senderAddress, gasLimit } = txProps;
    ...
    const userId = user && user.blockchainUserId ? user.blockchainUserId : 'unknown';
    ...
    const sig = decodedABI.params.find((param) => param.name === '_subjectSig').value;
    ...
    await redis.zadd('relayTxAttempts', Math.floor(Date.now() / 1000), JSON.stringify({
    ...
    try {
        ...
        if (!sendToNethermindOnly) {
            ...
            relayPromises.push(createAndSendTransaction(wallet, contractAddress, '0x00', web3, req.logger, gasLimit, encodedABI));
        }
        // send to nethermind
        ...
        if (sendToNethermindOnly) {
            ...
            relayPromises.push(relayToNethermindWithTimeout(nethermindEncodedABI, nethermindContractAddress, gasLimit));
        }
        ...
        if (relayTxs.length === 1) {
            ...
            logger.info(`txRelay - poaTx: ${JSON.stringify(poaTx?.txParams)} | nethermindTx: ${JSON.stringify(nethermindTx?.txParams)}`);
            if (sendToNethermindOnly) {
        ...
        await redis.hset('txHashToSenderAddress', txReceipt.transactionHash, senderAddress);
    }
    catch (e) {
...
const filterReplicaSetUpdates = (decodedABI, senderAddress) => {
    ...
    if (decodedABI.name === 'updateReplicaSet') {
        ...
        isFirstReplicaSetConfig = decodedABI.params.find((param) => param.name === '_oldPrimaryId' && param.value === '0');
    }
    ...
    if (isReplicaSetTransaction && !isFirstReplicaSetConfig) {
        ...
        if (transactionRateLimiter.updateReplicaSetReconfiguration >
            UPDATE_REPLICA_SET_RECONFIGURATION_LIMIT) {
            throw new Error('updateReplicaSet rate limit reached');
        }
        if (UPDATE_REPLICA_SET_WALLET_WHITELIST.length > 0 &&
    ...
    for (const wallet of relayerWallets) {
        ...
        if (parseInt(balance) < minimumBalance) {
            ...
            if (ENVIRONMENT === 'development') {
                ...
                }, wallet.publicKey, minimumBalance, primaryWeb3, logger);
...
const createAndSendTransaction = async (sender, receiverAddress, value, web3, logger, gasLimit = null, data = null) => {
    ...
    logger.info(`createAndSendTransaction ok txhash: ${signedTx.transactionHash} took: ${took}`);
...
//
async function relayToNethermindWithTimeout(encodedABI, contractAddress, gasLimit) {
    return Promise.race([
...
async function relayToNethermind(encodedABI, contractAddress, gasLimit) {
    logger.info(`txRelay - relayToNethermind input params: ${encodedABI} ${contractAddress} ${gasLimit}`);
    ...
    try {
        ...
        const signedTx = await nethermindWeb3.eth.accounts.signTransaction(transaction, privateKey);
        inFlight++;
        const myDepth = inFlight;
        logger.info(`relayToNethermind sending txhash: ${signedTx.transactionHash} num: ${myDepth}`);
        const receipt = await nethermindWeb3.eth.sendSignedTransaction(signedTx.rawTransaction);
        logger.info(`txRelay - relayToNethermind receipt: ${JSON.stringify(receipt)}`);
        receipt.blockNumber += config.get('finalPOABlock');
        const end = new Date().getTime();
        const took = end - start;
        inFlight--;
        logger.info(`relayToNethermind ok txhash: ${signedTx.transactionHash} num: ${myDepth} took: ${took} pending: ${inFlight}`);
        return {
