...
const redisClient = new Redis(config.get('redisPort'), config.get('redisHost'), { showFriendlyErrorStack: config.get('environment') !== 'production' });
const sigUtil = require('eth-sig-util');
const { generators } = require('@audius/sdk/src/data-contracts/signatureSchemas.js');
...
const isIPWhitelisted = (ip, req) => {
    ...
    req.logger.debug(`isIPWhitelisted - isWhitelisted: ${isWhitelisted}, isFromContentNode: ${isFromContentNode}`);
 ...
 */
const getRateLimiter = ({ prefix, max, expiry = DEFAULT_EXPIRY, keyGenerator = (req) => getIP(req).ip, handler, message, skip }) => {
    return rateLimit({
...
const recoverSigner = (encodedABI) => {
    const decodedABI = decodeABI(encodedABI);
    const data = generators.getManageEntityData(config.get('acdcChainId'), config.get('entityManagerAddress'), decodedABI.userId, decodedABI.entityType, decodedABI.entityId, decodedABI.action, decodedABI.metadata, decodedABI.nonce);
...
const getRelayBlocklistMiddleware = (req, res, next) => {
    ...
    if (blocklist && blocklist.includes(signer)) {
        sendResponse(req, res, errorResponseRateLimited({
    ...
    return getRateLimiter({
        ...
        handler: (req, res) => {
            try {
                const key = getEntityManagerActionKey(req.body.encodedABI);
                const signer = recoverSigner(req.body.encodedABI);
                req.logger.error({ _signer: signer, isApp: req.isFromApp }, `Rate limited sender ${signer} performing ${key}`);
            }
            catch (error) {
