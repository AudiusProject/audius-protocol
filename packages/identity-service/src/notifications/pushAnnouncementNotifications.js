...
async function pushAnnouncementNotifications() {
  ...
  try {
    ...
    logger.info(
      `pushAnnouncementNotifications - processed ${numProcessedNotifs} notifs in ${
        Date.now() - time
      }ms`
    )
  } catch (e) {
...
async function processAnnouncement(notif, tx) {
  ...
  const pushedNotifRecord =
    await models.PushedAnnouncementNotifications.findAll({
      where: {
...
async function _pushAnnouncement(notif, tx) {
  ...
  await Promise.all(
    validDeviceRecords.map(async (device) => {
