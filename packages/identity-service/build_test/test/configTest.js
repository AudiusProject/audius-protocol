...
describe('convict configuration test', function () {
    ...
    it('setting a value through env key, and ensuring the new value is set in config', function () {
        ...
        for (var key in schema) {
            ...
            assert(config.get(key) === validValue, `The config key '${key}' with format type '${schema[key].format}' is still retaining its old value of '${oldValue}' instead of '${validValue}'`);
        }
    });
});
