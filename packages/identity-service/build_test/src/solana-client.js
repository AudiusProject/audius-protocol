...
const TRACK_LISTEN_PROGRAM = config.get('solanaTrackListenCountAddress')
    ...
    : null;
const INSTRUCTIONS_PROGRAM = new solanaWeb3.PublicKey('Sysvar1nstructions1111111111111111111111111');
const CLOCK_PROGRAM = new solanaWeb3.PublicKey('SysvarC1ock11111111111111111111111111111111');
class TrackData {
...
async function createTrackListenInstructions({ privateKey, userId, trackId, source, location, connection }) {
    ...
    const signerGroup = new solanaWeb3.PublicKey(accInfo.data.toJSON().data.slice(1, 33)); // cut off version and eth address from valid signer data
    let sourceData;
    ...
    const trackData = new TrackData({
        ...
        trackId: trackId,
        source: sourceData,
        timestamp: (await getListenTimestamp(connection)) ||
    ...
    const serializedInstructionArgs = borsh.serialize(instructionSchema, instructionData);
    const secpInstruction = solanaWeb3.Secp256k1Program.createInstructionWithPublicKey({
        publicKey: pubKey,
