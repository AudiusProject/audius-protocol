...
const getAbuseData = async (handle, reqIP, abbreviated) => {
    const res = await axios.get(`${aaoEndpoint}/abuse/${handle}${abbreviated ? '?abbreviated=true' : ''}`, {
        headers: {
    ...
    const appliedFailRules = rules
        ...
        .map((r) => r.rule);
    const blockedFromRelay = appliedFailRules.some((r) => blockRelayAbuseErrorCodes.has(r));
    const blockedFromNotifications = appliedFailRules.some((r) => blockNotificationsErrorCodes.has(r));
    const blockedFromEmails = appliedFailRules.some((r) => blockEmailsErrorCodes.has(r));
    return {
        blockedFromRelay,
...
const detectAbuse = async (user, reqIP, abbreviated = false) => {
    ...
    try {
        ...
        logger.info(`detectAbuse: got info for user id ${user.blockchainUserId} handle ${user.handle}: ${JSON.stringify({
            appliedRules,
            blockedFromRelay,
            blockedFromNotifications,
            blockedFromEmails
    ...
    if (!!user.isBlockedFromRelay !== blockedFromRelay ||
        !!user.isBlockedFromNotifications !== blockedFromNotifications ||
        !!user.isBlockedFromEmails !== blockedFromEmails) {
        logger.info(`abuse: state changed for user [${user.handle}], blocked from relay: ${blockedFromRelay}, blocked from notifs: [${blockedFromNotifications}, blocked from emails: ${blockedFromEmails}]`);
        await user.update({
