...
module.exports = function (app) {
    ...
    app.get('/user_playlist_updates', handleResponse(async (req) => {
        ...
        try {
            ...
            return errorResponseServerError(`Unable to get user last playlist views for ${walletAddress}`);
        }
    }));
    /**
    ...
    app.post('/user_playlist_updates', authMiddleware, handleResponse(async (req) => {
        ...
        if (!walletAddress || !playlistId) {
            return errorResponseBadRequest('Please provide a wallet address and a playlist library item id');
        }
        try {
            ...
            if (!result)
                throw new Error(`Playlist updates for ${walletAddress} not found`);
            ...
            await models.UserEvents.update({ playlistUpdates }, { where: { walletAddress } });
            return successResponse({});
        }
        catch (e) {
            req.logger.error(e);
            console.log(e);
            return errorResponseServerError(`Unable to update user last playlist views for ${walletAddress} for playlist library item id ${playlistId}`);
        }
    }));
};
