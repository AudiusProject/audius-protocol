...
const { createTrackListenInstructions, getFeePayerKeypair } = require('../solana-client');
...
const getTrackListens = async (idList, timeFrame = undefined, startTime = undefined, endTime = undefined, limit = undefined, offset = undefined) => {
...
module.exports = function (app) {
    app.get('/tracks/listen/solana/status', handleResponse(async (req, res) => {
        ...
        const totalPercentSuccess = totalSubmissionCount === 0
            ? 1
        ...
        const recentSuccessPercent = recentSubmissionCount === 0
            ? 1
    ...
    app.post('/tracks/:id/listen', handleResponse(async (req, res) => {
        ...
        req.logger.debug(`TrackListen tx submission, trackId=${trackId} userId=${userId}, ${JSON.stringify(trackingRedisKeys)}`);
        await redis.incr(trackingRedisKeys.submission);
        await redis.zadd(TRACKING_LISTEN_SUBMISSION_KEY, Date.now(), Date.now() + entropy);
        let location;
        try {
            let clientIPAddress = (req.headers['x-forwarded-for'] || '').split(',').pop().trim() ||
            ...
            req.logger.error(`TrackListen location fetch failed: ${e}, trackId=${trackId} userId=${userId}, ${JSON.stringify(trackingRedisKeys)}`);
            location = {};
        }
        try {
            ...
            req.logger.debug(`TrackListen tx submission, trackId=${trackId} userId=${userId} - sendRawTransaction`);
            const transactionHandler = libs.solanaWeb3Manager.transactionHandler;
            const { res: solTxSignature, error } = await transactionHandler.handleTransaction({
                instructions,
                skipPreflight: false,
            ...
            if (error) {
                return errorResponseServerError(`TrackListens tx error, trackId=${trackId} userId=${userId} : ${error}`);
            }
            req.logger.debug(`TrackListen tx confirmed, ${solTxSignature} userId=${userId}, trackId=${trackId} `);
    ...
    app.post('/tracks/listens/:timeframe*?', handleResponse(async (req, res, next) => {
        ...
        const output = await getTrackListens(idList, time, startTime, endTime, limit, offset);
    ...
    app.get('/tracks/listens/:timeframe*?', handleResponse(async (req, res) => {
        ...
        const time = parseTimeframe(req.params.timeframe);
        const { limit, offset } = getPaginationVars(req.query.limit, req.query.offset);
        const output = await getTrackListens(idList, time, startTime, endTime, limit, offset);
        return successResponse(output);
    }));
    /*
    ...
    app.post('/tracks/trending/:time*?', handleResponse(async (req, res) => {
        ...
        const idList = body.track_ids;
        const { limit, offset } = getPaginationVars(body.limit, body.offset);
        const parsedListenCounts = await getTrendingTracks(idList, time, limit, offset);
