...
module.exports = function (app) {
    app.post('/transaction_metadata', authMiddleware, handleResponse(async (req, res, next) => {
    ...
    app.get('/transaction_metadata', authMiddleware, handleResponse(async (req, res, next) => {
        const userId = req.user.blockchainUserId;
        const { id } = req.query;
        if (!id) {
        ...
        if (!ids.every((id) => typeof id === 'string')) {
            return errorResponseBadRequest('Invalid transaction signatures specified');
        }
