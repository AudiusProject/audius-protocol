"use strict";
const { handleResponse, successResponse, errorResponseForbidden, errorResponseServerError } = require('../apiHelpers');
...
module.exports = function (app) {
    ...
    app.post('/cognito_webhook/flow', cognitoFlowMiddleware, handleResponse(async (req) => {
        ...
        if (event !== 'flow_session.status.updated')
    ...
    app.get('/cognito_recent_exists/:handle', handleResponse(async (req) => {
        ...
        if (records.length) {
            const timeDifferenceMilliseconds = Date.now() - new Date(records[0].updatedAt).getTime();
            return successResponse({
    ...
    app.post('/cognito_flow', authMiddleware, handleResponse(async (req) => {
        ...
        try {
            ...
            logger.error(`Request failed to Cognito. Request=${JSON.stringify({
                url,
                method,
                headers,
                body
