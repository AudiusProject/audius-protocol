...
module.exports = function (app) {
    app.get('/tiktok', handleResponse(async (req, res, next) => {
    ...
    app.post('/tiktok/access_token', cors(accessTokenCorsOptions), handleResponse(async (req, res, next) => {
        ...
        try {
            ...
            const { data: { access_token: accessToken, error_code: errorCode, description: errorMessage } } = accessTokenResponse.data;
            if (errorCode) {
                return errorResponseBadRequest(`Received error from tiktok oauth: ${errorCode} ${errorMessage}`);
            }
            // Fetch TikTok user from the TikTok API
            const userResponse = await axios.post(`https://open-api.tiktok.com/user/info/?access_token=${accessToken}`, {
                fields: [
                    'open_id',
                    'username',
                    'display_name',
            ...
            if (existingTikTokUser) {
                return errorResponseBadRequest(`Another Audius profile has already been authenticated with TikTok user @${tikTokUser.username}!`);
            }
            else {
     ...
     */
    app.post('/tiktok/associate', handleResponse(async (req, res, next) => {
        ...
        try {
            ...
            if (isUnassociated && handlesMatch) {
                ...
                if (tikTokObj.verified) {
                    const [encodedABI, contractAddress] = await audiusLibsInstance.User.updateIsVerified(userId, config.get('userVerifierPrivateKey'));
                ...
                req.logger.error(`TikTok profile does not exist or userId has already been set for uuid: ${uuid}`, tikTokObj);
                return errorResponseBadRequest('TikTok profile does not exist or userId has already been set');
            }
        }
        catch (err) {
            return errorResponseBadRequest(err);
