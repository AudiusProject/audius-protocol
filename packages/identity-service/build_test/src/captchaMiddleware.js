...
const verifyAndRecordCaptcha = async ({ token, walletAddress, url, logger, captcha }) => {
    ...
    if (token) {
        ...
        if (!ok)
            logger.warn(`CAPTCHA - Failed captcha with score=${score} for wallet=${walletAddress}`);
    }
    else {
...
async function captchaMiddleware(req, res, next) {
    if (!config.get('recaptchaServiceKey')) {
        req.logger.warn(`CAPTCHA - No service key found. Not calculating score at ${req.url} for wallet=${req.body.walletAddress}`);
    }
    else {
