...
function getMilestoneEntity(notification, metadata) {
    if (notification.type === NotificationType.MilestoneFollow)
        return undefined;
    const type = notification.actions[0].actionEntityType;
...
function formatRemixCreate(notification, metadata) {
    const trackId = notification.entityId;
    const parentTrackAction = notification.actions.find((action) => action.actionEntityType === actionEntityTypes.Track &&
...
function formatRemixCosign(notification, metadata) {
    const trackId = notification.entityId;
    const parentTrackUserAction = notification.actions.find((action) => action.actionEntityType === actionEntityTypes.User);
...
const pushNotificationMessagesMap = {
    ...
    [notificationTypes.Create.base](notification) {
        ...
        if (notification.entity.type === actionEntityTypes.Track &&
