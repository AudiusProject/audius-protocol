        ...
        try {
            ...
            for (const respUser of response.data.data) {
                ...
                if (respUser.handle === updateUser.handle) {
                    const missingUserId = respUser.user_id;
                    await models.User.update({ blockchainUserId: missingUserId }, { where: { walletAddress, handle: updateUser.handle } });
                    logger.info(`Updated wallet ${walletAddress} to blockchainUserId: ${missingUserId}, ${updateUser.handle}`);
...
async function bulkGetSubscribersFromDiscovery(userIds) {
    ...
    try {
        ...
        const response = await axios.post(`${discoveryProvider.discoveryProviderEndpoint}/v1/full/users/subscribers`, { ids: ids });
...
const shouldReadSubscribersFromDiscovery = (optimizelyClient) => {
    ...
    return getFeatureFlag(optimizelyClient, FEATURE_FLAGS.READ_SUBSCRIBERS_FROM_DISCOVERY_ENABLED);
};
/**
...
async function shouldNotifyUser(notificationTarget, prop, tx = null) {
    ...
    if (tx)
        mobileQuery.transaction = tx;
    const userNotifSettingsMobile = await models.UserNotificationMobileSettings.findOne(mobileQuery);
    ...
    if (tx)
        browserPushQuery.transaction = tx;
    const userNotifBrowserPushSettings = await models.UserNotificationBrowserSettings.findOne(browserPushQuery);
    const notifyBrowserPush = (userNotifBrowserPushSettings && userNotifBrowserPushSettings[prop]) ||
        false;
