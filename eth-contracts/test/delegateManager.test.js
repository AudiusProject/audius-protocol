...
contract('DelegateManager', async (accounts) => {
  ...
  const increaseRegisteredProviderStake = async (increase, account) => {
    ...
    await token.approve(stakingAddress, increase, { from: account })

    let tx = await serviceProviderFactory.increaseStake(increase, {
      from: account
    })
  }
  ...
  const decreaseRegisteredProviderStake = async (decrease, account) => {
    try {
      ...
      await serviceProviderFactory.requestDecreaseStake(decrease, {
        from: account
      })
      let requestInfo =
      ...
      // Cancel request
      await serviceProviderFactory.cancelDecreaseStakeRequest(account, {
        from: account
      })
      throw e
    }
  ...
  */
  const getTotalDelegatorStake = async (delegator) => {
    ...
    for (const sp of spsArray) {
      let totalDelegatedToOwner =
        await delegateManager.getDelegatorStakeForServiceProvider(delegator, sp)
      totalDelegatorStake = totalDelegatorStake.add(totalDelegatedToOwner)
    }

    let newTotalFromContract = await delegateManager.getTotalDelegatorStake(
      delegator
    )
    assert.isTrue(
  ...
  const getAccountStakeInfo = async (account, print = false) => {
    ...
    let spDecreaseRequest =
      await serviceProviderFactory.getPendingDecreaseStakeRequest(account)

    let delegatedStake =
      await delegateManager.getTotalDelegatedToServiceProvider(account)
    let lockedUpDelegatorStake =
      await delegateManager.getTotalLockedDelegationForServiceProvider(account)
    let delegatorInfo = {}
    ...
    for (var i = 0; i < delegators.length; i++) {
      ...
      let amountDelegatedtoSP =
        await delegateManager.getDelegatorStakeForServiceProvider(
          delegators[i],
          account
        )
      let pendingUndelegateRequest =
        await delegateManager.getPendingUndelegateRequest(delegators[i])
    ...
    if (print) {
      console.log(
        `${account} SpFactory: ${spFactoryStake}, DelegateManager: ${delegatedStake}`
      )
      console.log(
        `${account} Outside Stake: ${outsideStake} Staking: ${totalInStakingContract}`
      )
      console.log(
        `(Staking) vs (DelegateManager + SPFactory) Stake discrepancy: ${stakeDiscrepancy}`
      )
  ...
  describe('Delegation tests', () => {
    ...
    beforeEach(async () => {
      ...
      await token.transfer(stakerAccount2, INITIAL_BAL, {
        from: proxyDeployerAddress
      })
      // Transfer 1000 tokens to delegator
      await token.transfer(delegatorAccount1, INITIAL_BAL, {
        from: proxyDeployerAddress
      })
      ...
      await serviceProviderFactory.requestUpdateDeployerCut(
        stakerAccount,
        updatedCut,
        { from: stakerAccount }
      )
      ...
      await serviceProviderFactory.updateDeployerCut(stakerAccount2, {
        from: stakerAccount2
      })
      await serviceProviderFactory.updateDeployerCut(stakerAccount, {
        from: stakerAccount
      })
      ...
      assert.isTrue(
        info.deployerCut.eq(_lib.toBN(updatedCut)),
        'Expect updated cut'
      )
    ...
    it('Initial state + claim', async () => {
      ...
      let spStake = (
        await serviceProviderFactory.getServiceProviderDetails(stakerAccount)
      ).deployerStake
    ...
    it('Initial state + claim from different address', async () => {
      ...
      assert.isTrue(
        claimerAddress !== stakerAccount,
        'Expected different claimer account'
      )
      await delegateManager.claimRewards(stakerAccount, {
        from: claimerAddress
      })
    ...
    it('Claim zero for an address that has zero stake', async () => {
      ...
      assert.isTrue(
        claimerAddress !== stakerAccount,
        'Expected fake service provider address'
      )
      let preClaimInfo = await getAccountStakeInfo(fakeSPAddress)

      let tx = await delegateManager.claimRewards(fakeSPAddress, {
        from: claimerAddress
      })
      let postClaimInfo = await getAccountStakeInfo(fakeSPAddress)
      assert.isTrue(
        postClaimInfo.totalInStakingContract.eq(
          preClaimInfo.totalInStakingContract
        ),
        'Expect no reward for an account claiming'
      )
      await expectEvent.inTransaction(tx.tx, DelegateManager, 'Claim', {
    ...
    it('Single delegator basic operations', async () => {
      ...
      await token.approve(stakingAddress, delegateAmount, {
        from: delegatorAccount1
      })

      await delegateManager.delegateStake(stakerAccount, delegateAmount, {
        from: delegatorAccount1
      })
      ...
      assert.isTrue(delegatorFound, 'Delegator found in array')
      assert.isTrue(
      ...
      await token.approve(stakingAddress, increaseAmount, {
        from: delegatorAccount1
      })
      await delegateManager.delegateStake(stakerAccount, increaseAmount, {
        from: delegatorAccount1
      })
      assert.isTrue(
        (await getTotalDelegatorStake(delegatorAccount1)).eq(
          delegatedStake.add(increaseAmount)
        ),
        'Confirm increase'
      )
      ...
      let undelegateRequestInfo =
        await delegateManager.getPendingUndelegateRequest(delegatorAccount1)
      assert.isTrue(
      ...
      let totalLockedDelegation =
        await delegateManager.getTotalLockedDelegationForServiceProvider(
          stakerAccount
        )
      assert.isTrue(
      ...
      await _lib.assertRevert(
        delegateManager.requestUndelegateStake(stakerAccount, delegatedStake, {
          from: delegatorAccount1
        }),
      ...
      undelegateRequestInfo = await delegateManager.getPendingUndelegateRequest(
        delegatorAccount1
      )
    ...
    it('Single delegator + claim', async () => {
      ...
      // Approve staking transfer
      await token.approve(stakingAddress, initialDelegateAmount, {
        from: delegatorAccount1
      })
      ...
      let delegateRewardsPriorToSPCut = delegatedStake
        .mul(totalRewards)
        .div(totalValueOutsideStaking)
      let spDeployerCut = delegateRewardsPriorToSPCut
        .mul(deployerCut)
        .div(deployerCutBase)
      ...
      assert.isTrue(
        ...
        'Expected SP stake matches found value'
      )
      assert.isTrue(
        finalDelegateStake.eq(expectedDelegateStake),
        'Expected delegate stake matches found value'
      )
    })
    ...
    it('Undelegate after SP has deregistered', async () => {
      ...
      assert.isTrue(
        accountInfo.delegatorInfo[delegatorAccount1].amountDelegated.eq(
          initialDelegateAmount
        ),
      ...
      let finalDelegateStake = await getTotalDelegatorStake(delegatorAccount1)
      assert.isTrue(
        finalDelegateStake.eq(_lib.toBN(0)),
        'No remaining stake expected'
      )
    })
    ...
    it('Register 3rd service provider after round has completed', async () => {
      ...
      // Transfer 1000 tokens to delegator
      await token.transfer(stakerAccount3, INITIAL_BAL, {
        from: proxyDeployerAddress
      })
      // Fund new claim
      await claimsManager.initiateRound({ from: stakerAccount })
      // Get rewards
      await delegateManager.claimRewards(stakerAccount, { from: stakerAccount })
      await delegateManager.claimRewards(stakerAccount2, {
        from: stakerAccount2
      })
      ...
      assert.isTrue(
        info.totalInStakingContract.eq(DEFAULT_AMOUNT),
        'Expect stake'
      )
      assert.isTrue(
        info.spFactoryStake.eq(DEFAULT_AMOUNT),
        'Expect sp factory stake'
      )
    })
    ...
    it('Single delegator + claim + slash', async () => {
      ...
      // Slash 30% of total
      const totalInStakingContract = await staking.totalStakedFor(
        slasherAccount
      )
      const slashAmountVal = _lib.audToWei(
        (_lib.fromWei(totalInStakingContract) * 30) / 100
      )
      ...
      assert.equal(
        stakeDiscrepancy,
        0,
        'Equal tokens expected inside/outside Staking'
      )
      assert.isTrue(
        totalStaked.eq(tokensAtStakingAddress),
        'Expect equivalency between Staking contract and ERC'
      )
      assert.isTrue(
        totalInStakingAfterSlash.eq(outsideStake),
        'Expected SP/delegatemanager to equal staking'
      )
      assert.isTrue(
        totalInStakingContract.sub(slashAmount).eq(totalInStakingAfterSlash),
        'Expected slash value'
      )
    })
    ...
    it('Slash restrictions', async () => {
      ...
      await delegateManager.delegateStake(stakerAccount2, DEFAULT_AMOUNT, {
        from: delegatorAccount1
      })
    ...
    it('40 delegators to one SP + claim', async () => {
      ...
      if (accounts.length < numDelegators) {
        ...
        console.log(
          `Insufficient accounts found - required ${numDelegators}, found ${accounts.length}`
        )
        return
      }

      let delegateAccountOffset = 4
      let delegatorAccounts = accounts.slice(
        delegateAccountOffset,
        delegateAccountOffset + numDelegators
      )
      let totalDelegationAmount = DEFAULT_AMOUNT
      let singleDelegateAmount = totalDelegationAmount.div(
        web3.utils.toBN(numDelegators)
      )
      await Promise.all(
        delegatorAccounts.map(async (delegator) => {
          // Transfer 1000 tokens to each delegator
          await token.transfer(delegator, INITIAL_BAL, {
            from: proxyDeployerAddress
          })
          // Approve staking transfer
          await token.approve(stakingAddress, singleDelegateAmount, {
            from: delegator
          })
      ...
      let totalSPStakeAfterDelegation = await staking.totalStakedFor(
        stakerAccount
      )
      let expectedTotalStakeAfterDelegation = totalStakedForSP.add(
      ...
      let fundingAmount = await claimsManager.getFundsPerRound()
      let totalExpectedRewards = totalStakedForSP
        .mul(fundingAmount)
        .div(totalStake)
      ...
      await Promise.all(
        delegatorAccounts.map(async (delegator) => {
          ...
          let delegateRewardsPriorToSPCut = delegatorStake
            .mul(totalExpectedRewards)
            .div(totalValueOutsideStaking)
          let spDeployerCut = delegateRewardsPriorToSPCut
            .mul(deployerCut)
            .div(deployerCutBase)
          ...
          totalDelegateStakeIncrease =
            totalDelegateStakeIncrease.add(delegateRewards)
        })
      )
      ...
      await Promise.all(
        delegatorAccounts.map(async (delegator) => {
          let finalDelegatorStake =
            await delegateManager.getDelegatorStakeForServiceProvider(
              delegator,
              stakerAccount
            )
          let expectedDelegatorStake =
            expectedDelegateStakeDictionary[delegator]
          assert.isTrue(
      ...
      let finalRewards = postClaimInfo.totalInStakingContract.sub(
        preClaimInfo.totalInStakingContract
      )
      assert.isTrue(
        finalRewards.eq(totalExpectedRewards),
        `Expected ${totalExpectedRewards.toString()} in rewards, found ${finalRewards.toString()}`
      )
    ...
    it('Undelegate partial amount', async () => {
      ...
      await delegateManager.undelegateStake({ from: delegatorAccount1 })
      let delegatorBalanceAfterUndelegation = await token.balanceOf(
        delegatorAccount1
      )
      assert.isTrue(
        delegatorBalanceAfterUndelegation
          .sub(undelegateAmount)
          .eq(delegatorTokenBalance),
        'Expect funds to be returned'
      )

      let expectedStake = initialDelegateAmount.sub(undelegateAmount)
      delStakeForSP = await delegateManager.getDelegatorStakeForServiceProvider(
        delegatorAccount1,
        stakerAccount
      )
    ...
    it('Fail when undelegating zero stake', async () => {
      ...
      // Submit request to undelegate
      await _lib.assertRevert(
        delegateManager.requestUndelegateStake(stakerAccount, 0, {
          from: delegatorAccount1
        }),
    ...
    it('Single delegator + undelegate + claim', async () => {
      ...
      let preRewardDelegation =
        preRewardInfo.delegatorInfo[delegatorAccount1].amountDelegated
      let postRewardDelegation =
        postRewardInfo.delegatorInfo[delegatorAccount1].amountDelegated
      assert.isTrue(
    ...
    it('Single delegator to invalid SP', async () => {
      ...
      await _lib.assertRevert(
        delegateManager.delegateStake(accounts[8], initialDelegateAmount, {
          from: delegatorAccount1
        }),
    ...
    it('Validate undelegate request restrictions', async () => {
      ...
      await _lib.assertRevert(
        delegateManager.requestUndelegateStake(stakerAccount, 10, {
          from: delegatorAccount1
        }),
      ...
      await _lib.assertRevert(
        delegateManager.requestUndelegateStake(
          stakerAccount,
          initialDelegateAmount.add(_lib.toBN(10)),
          { from: delegatorAccount1 }
        ),
    ...
    it('3 delegators + pending claim + undelegate restrictions', async () => {
      ...
      await token.transfer(delegatorAccount2, INITIAL_BAL, {
        from: proxyDeployerAddress
      })
      await token.transfer(delegatorAccount3, INITIAL_BAL, {
        from: proxyDeployerAddress
      })
      let initialDelegateAmount = _lib.audToWeiBN(60)
      ...
      assert.isTrue(
        web3.utils
          .toBN(currentBlockNum)
          .gte(undelegateRequestInfo.lockupExpiryBlock),
    ...
    it('Undelegate after slash below bounds', async () => {
      ...
      // Perform slash functions
      await _lib.slash(
        _lib.audToWei(_lib.fromWei(slashAmount)),
        slasherAccount,
        governance,
        delegateManagerKey,
        guardianAddress
      ...
      assert.isFalse(spDetails.validBounds, 'Bound violation expected')
    ...
    it('Slash below sp bounds', async () => {
      ...
      await _lib.assertRevert(
        increaseRegisteredProviderStake(diffAmount, stakerAccount),
      ...
      assert.isTrue(
        spDetails.totalInStakingContract.eq(
          spDetailsAfterClaim.totalInStakingContract
        ),
      ...
      await increaseRegisteredProviderStake(increase, stakerAccount)
      // Validate increase
      spDetails = await serviceProviderFactory.getServiceProviderDetails(
        stakerAccount
      )
      assert.isTrue(spDetails.validBounds, 'Valid bound expected')
    })
    ...
    it('Delegator increase/decrease + SP direct stake bound validation', async () => {
      ...
      await token.transfer(delegatorAccount1, failedIncreaseAmount, {
        from: proxyDeployerAddress
      })
      // Approve staking transfer
      await token.approve(stakingAddress, failedIncreaseAmount, {
        from: delegatorAccount1
      })
      await _lib.assertRevert(
        delegateManager.delegateStake(stakerAccount, failedIncreaseAmount, {
          from: delegatorAccount1
        }),
        'Maximum stake amount exceeded'
      )
      ...
      let infoAfterSecondEndpoint = await getAccountStakeInfo(
        stakerAccount,
        false
      )
      assert.isTrue(
        infoAfterSecondEndpoint.totalInStakingContract.eq(
          infoAfterDecrease.totalInStakingContract
        ),
    ...
    it('Maximum delegators', async () => {
      ...
      for (var i = 0; i < delegatorAccounts.length; i++) {
        ...
        if (i === delegatorAccounts.length - 1) {
          await _lib.assertRevert(
            delegateManager.delegateStake(stakerAccount, singleDelegateAmount, {
              from: delegator
            }),
    ...
    it('Min delegator stake per SP', async () => {
      ...
      // Approve staking transfer
      await token.approve(stakingAddress, minDelegateStake, {
        from: delegatorAccount1
      })

      // Delegate valid min to SP 1
      await delegateManager.delegateStake(stakerAccount, minDelegateStake, {
        from: delegatorAccount1
      })

      // Delegate invalid min for SP 2
      let invalidMinStake = _lib.toBN(_lib.audToWei(1))
      await token.approve(stakingAddress, invalidMinStake, {
        from: delegatorAccount1
      })
      await _lib.assertRevert(
        delegateManager.delegateStake(stakerAccount2, invalidMinStake, {
          from: delegatorAccount1
        }),
      ...
      await delegateManager.delegateStake(stakerAccount2, minDelegateStake, {
        from: delegatorAccount1
      })
      assert.isTrue(
        (
    ...
    it('Min delegator stake per SP - with DelegateManagerV2', async () => {
      ...
      assert.isTrue(
        (
          await delegateManager.getSPMinDelegationAmount(stakerAccount)
        ).isZero(),
        'Expect spMinDelegationAmount = 0'
      )
      ...
      let tx = await delegateManager.updateSPMinDelegationAmount(
        stakerAccount,
      ...
      await expectEvent.inTransaction(
        ...
        'SPMinDelegationAmountUpdated',
        {
          ...
          _spMinDelegationAmount: spMinDelegationAmount
        }
      )
      assert.isTrue(
        (await delegateManager.getSPMinDelegationAmount(stakerAccount)).eq(
          _lib.toBN(spMinDelegationAmount)
        ),
        'Expect spMinDelegationAmount to be changed'
      )
      ...
      await _lib.assertRevert(
        delegateManager.delegateStake(stakerAccount, invalidMinStake, {
          from: delegatorAccount1
        }),
        'Minimum delegation amount'
      )
      ...
      await token.approve(stakingAddress, validMinStake, {
        from: delegatorAccount1
      })
      await delegateManager.delegateStake(stakerAccount, validMinStake, {
        from: delegatorAccount1
      })
      assert.isTrue(
        (
        ...
        ).eq(_lib.toBN(spMinDelegationAmount)),
        'Expect min delegate stake'
      )
    })
    ...
    it('Min delegate stake verification', async () => {
      ...
      let tx = await delegateManager.requestUndelegateStake(
        stakerAccount,
        failUndelegateAmount,
        { from: delegatorAccount1 }
      )
      let lockupExpiryBlock = _lib
        .toBN(tx.receipt.blockNumber)
        .add(lockupDuration)
      await expectEvent.inTransaction(
        tx.tx,
        DelegateManager,
      ...
      // Cancel request
      tx = await delegateManager.cancelUndelegateStakeRequest({
        from: delegatorAccount1
      })
      await expectEvent.inTransaction(
        tx.tx,
      ...
      assert.equal(
        _lib.fromBN(await getTotalDelegatorStake(delegatorAccount1)),
        0,
        'No stake expected'
      )
      assert.equal(
        (await delegateManager.getDelegatorsList(stakerAccount)).length,
        0,
        'No delegators expected'
      )
      await expectEvent.inTransaction(
        tx.tx,
        DelegateManager,
    ...
    it('Cancel undelegate stake resets state', async () => {
      ...
      await delegateManager.cancelUndelegateStakeRequest({
        from: delegatorAccount1
      })

      let undelegateRequestInfoAfterCancel =
        await delegateManager.getPendingUndelegateRequest(delegatorAccount1)
      let lockedUpStakeForSpAfterCancel =
        await delegateManager.getTotalLockedDelegationForServiceProvider(
          stakerAccount
        )

      assert.isTrue(
        undelegateRequestInfoAfterCancel.target === _lib.addressZero,
        'Expect zero address'
      )
      assert.isTrue(
        undelegateRequestInfoAfterCancel.amount.eq(_lib.toBN(0)),
        'Expect 0 pending in undelegate request'
      )
      assert.isTrue(
        undelegateRequestInfoAfterCancel.lockupExpiryBlock.eq(_lib.toBN(0)),
        'Expect 0 pending in undelegate request'
      )
      assert.isTrue(
        lockedUpStakeForSpAfterCancel.eq(
          lockedUpStakeForSp.sub(undelegateRequestInfo.amount)
        ),
        'Expect decrease in locked up stake for SP'
      )
    })
    ...
    it('Caller restriction verification', async () => {
      ...
      await _lib.assertRevert(
        delegateManager.updateRemoveDelegatorLockupDuration(10, {
          from: accounts[3]
        }),
        'Only callable by Governance contract'
      )
      await _lib.assertRevert(
        delegateManager.updateRemoveDelegatorEvalDuration(10, {
          from: accounts[3]
        }),
        'Only callable by Governance contract'
      )
    ...
    it('Deregister max stake violation while delegator has an undelegate request pending', async () => {
      let serviceTypeInfo = await serviceTypeManager.getServiceTypeInfo(
        testDiscProvType
      )
      ...
      let ids = await serviceProviderFactory.getServiceProviderIdsFromAddress(
        stakerAccount,
        testDiscProvType
      )
      ...
      let spInfo = await serviceProviderFactory.getServiceProviderDetails(
        stakerAccount
      )
      ...
      await time.advanceBlockTo(deregisterRequestInfo.lockupExpiryBlock)
      await serviceProviderFactory.decreaseStake({ from: stakerAccount })
      spInfo = await serviceProviderFactory.getServiceProviderDetails(
        stakerAccount
      )
      assert.isTrue(
        spInfo.deployerStake.eq(spInfo.minAccountStake),
        'Expect min stake for deployer'
      )
      ...
      await token.transfer(delegatorAccount1, delegationAmount, {
        from: proxyDeployerAddress
      })
      await token.approve(stakingAddress, delegationAmount, {
        from: delegatorAccount1
      })
      await delegateManager.delegateStake(stakerAccount, delegationAmount, {
        from: delegatorAccount1
      })
      spInfo = await getAccountStakeInfo(stakerAccount)
      assert.isTrue(
        spInfo.maxAccountStake.eq(spInfo.totalActiveStake),
        'Expect max to be reached after delegation'
      )
      ...
      await _lib.assertRevert(
        delegateManager.removeDelegator(stakerAccount, delegatorAccount1, {
          from: delegatorAccount1
        }),
      ...
      // Remove delegator
      await delegateManager.requestRemoveDelegator(
        stakerAccount,
        delegatorAccount1,
        { from: stakerAccount }
      )

      let requestTargetBlock =
        await delegateManager.getPendingRemoveDelegatorRequest(
          stakerAccount,
          delegatorAccount1
        )
      ...
      await delegateManager.removeDelegator(stakerAccount, delegatorAccount1, {
        from: stakerAccount
      })

      let stakeAfterRemoval =
        await delegateManager.getDelegatorStakeForServiceProvider(
          delegatorAccount1,
          stakerAccount
        )
      let delegatorsList = await delegateManager.getDelegatorsList(
        stakerAccount
      )
      pendingUndelegateRequest =
        await delegateManager.getPendingUndelegateRequest(delegatorAccount1)
      assert.isTrue(
        stakeAfterRemoval.eq(_lib.toBN(0)),
        'Expect 0 delegated stake'
      )
      ...
      assert.isTrue(
        diff.eq(delegationAmount),
        'Expect full delegation amount to be refunded'
      )
      ...
      let spID2 = await serviceProviderFactory.getServiceProviderIdFromEndpoint(
        testEndpoint
      )
    ...
    it('removeDelegator validation', async () => {
      ...
      await token.transfer(delegatorAccount2, delegationAmount, {
        from: proxyDeployerAddress
      })
      await token.approve(stakingAddress, delegationAmount, {
        from: delegatorAccount1
      ...
      let removeReqDuration =
        await delegateManager.getRemoveDelegatorLockupDuration()
      let removeReqEvalDuration =
        await delegateManager.getRemoveDelegatorEvalDuration()

      // Remove delegator
      let tx = await delegateManager.requestRemoveDelegator(
        stakerAccount,
        delegatorAccount1,
        { from: stakerAccount }
      )
      ...
      // Move to valid block and actually perform remove
      await time.advanceBlockTo(requestTargetBlock)
      tx = await delegateManager.removeDelegator(
        stakerAccount,
        delegatorAccount1,
        { from: stakerAccount }
      )
      await expectEvent.inTransaction(
        ...
        'RemoveDelegatorRequestEvaluated',
        {
          _serviceProvider: stakerAccount,
          _delegator: delegatorAccount1,
          _unstakedAmount: delegationAmount
        }
      )
      ...
      await _lib.assertRevert(
        delegateManager.requestRemoveDelegator(
          stakerAccount,
          delegatorAccount2,
          { from: stakerAccount }
        ),
      ...
      // Request removal
      tx = await delegateManager.requestRemoveDelegator(
        stakerAccount,
        delegatorAccount2,
        { from: stakerAccount }
      )
      ...
      assert.isTrue(
        requestTargetBlock.eq(expectedTarget),
        'Target block unexpected'
      )

      // Call from wrong account
      await _lib.assertRevert(
        delegateManager.cancelRemoveDelegatorRequest(
          stakerAccount,
          delegatorAccount2
        ),
      ...
      tx = await delegateManager.cancelRemoveDelegatorRequest(
        stakerAccount,
        delegatorAccount2,
        { from: stakerAccount }
      )
      ...
      let requestTargetBlockAfterCancel =
        await delegateManager.getPendingRemoveDelegatorRequest(
          stakerAccount,
          delegatorAccount2
        )
      assert.isTrue(
        requestTargetBlockAfterCancel.eq(_lib.toBN(0)),
        'Expect reset'
      )
      ...
      await _lib.assertRevert(
        delegateManager.removeDelegator(stakerAccount, delegatorAccount2, {
          from: stakerAccount
        }),
    ...
    describe('Service provider decrease stake behavior', async () => {
      it('claimReward disabled if no active stake for SP', async () => {
        ...
        assert.isTrue(
          acctInfo.totalInStakingContract.eq(DEFAULT_AMOUNT),
          'Expect default in staking'
        )
        assert.isTrue(
          acctInfo.spFactoryStake.eq(DEFAULT_AMOUNT),
          'Expect default in sp factory'
        )
      })
      ...
      it('Re-registration after removing all stake and claim', async () => {
        ...
        assert.isFalse(
          await claimsManager.claimPending(stakerAccount),
          'Expect no claim pending'
        )
        // Withdraw all stake
        await serviceProviderFactory.decreaseStake({ from: stakerAccount })
        let acctInfo = await getAccountStakeInfo(stakerAccount)
        assert.isTrue(
          acctInfo.totalInStakingContract.eq(_lib.toBN(0)),
          'Expect default in staking'
        )
        assert.isTrue(
          acctInfo.spFactoryStake.eq(_lib.toBN(0)),
          'Expect default in sp factory'
        )
        assert.isTrue(
          acctInfo.numberOfEndpoints.eq(_lib.toBN(0)),
          'Expect no endpoints in sp factory'
        )
      ...
      it('Balance tracking inconsistency', async () => {
        ...
        await token.transfer(stakerAccount4, INITIAL_BAL, {
          from: proxyDeployerAddress
        })
        await token.transfer(stakerAccount5, INITIAL_BAL, {
          from: proxyDeployerAddress
        })
        await token.transfer(stakerAccount6, INITIAL_BAL, {
          from: proxyDeployerAddress
        })
        let totalStakedForAccount = await staking.totalStakedFor(stakerAccount)
        await _lib.registerServiceProvider(
        ...
        let sp2Stake = (
          await serviceProviderFactory.getServiceProviderDetails(stakerAccount2)
        ).deployerStake
        let sp2DelegatedStake =
          await delegateManager.getTotalDelegatedToServiceProvider(
            stakerAccount2
          )
        ...
        let tx = await delegateManager.claimRewards(stakerAccount, {
          from: stakerAccount
        })
        tx = await delegateManager.claimRewards(stakerAccount2, {
          from: stakerAccount2
        })
        tx = await delegateManager.claimRewards(stakerAccount3, {
          from: stakerAccount2
        })
        tx = await delegateManager.claimRewards(stakerAccount4, {
          from: stakerAccount2
        })
        tx = await delegateManager.claimRewards(stakerAccount5, {
          from: stakerAccount2
        })
        tx = await delegateManager.claimRewards(stakerAccount6, {
          from: stakerAccount2
        })
        ...
        tx = await delegateManager.claimRewards(stakerAccount, {
          from: stakerAccount
        })
        tx = await delegateManager.claimRewards(stakerAccount2, {
          from: stakerAccount2
      ...
      it('Slashing inconsistency', async () => {
        ...
        // Delegate to staker2 from staker1
        await delegateManager.delegateStake(stakerAccount3, delegateAmount, {
          from: stakerAccount2
        })
        let totalBalanceInSPFactoryPreSlash = (
          await serviceProviderFactory.getServiceProviderDetails(stakerAccount3)
        ).deployerStake
        let sp3DelegatedStakePreSlash =
          await delegateManager.getTotalDelegatedToServiceProvider(
            stakerAccount3
          )
        let totalBalanceInStakingPreSlash = await staking.totalStakedFor(
          stakerAccount3
        )
        // Perform slash
        ...
        let totalBalanceInStakingAfterSlash = await staking.totalStakedFor(
          stakerAccount3
        )
        let sp3NewDelegateStake = totalBalanceInStakingAfterSlash
          .mul(sp3DelegatedStakePreSlash)
          .div(totalBalanceInStakingPreSlash)
        let totalDelegatedStakeDecrease =
          sp3DelegatedStakePreSlash.sub(sp3NewDelegateStake)
        let totalStakeDecrease = totalBalanceInStakingPreSlash.sub(
          totalBalanceInStakingAfterSlash
        )
        let totalSPFactoryBalanceDecrease = totalStakeDecrease.sub(
          totalDelegatedStakeDecrease
        )
        let expectedSp3DeployerStake = totalBalanceInSPFactoryPreSlash.sub(
          totalSPFactoryBalanceDecrease
        )
      ...
      it('Decrease in reward for pending stake decrease', async () => {
        ...
        let stakingDiff = info2.totalInStakingContract.sub(
          info.totalInStakingContract
        )
        let spFactoryDiff = info2.spFactoryStake.sub(info.spFactoryStake)
        assert.isTrue(
          stakingDiff.eq(expectedIncrease),
          'Expected increase not found in Staking.sol'
        )
        assert.isTrue(
          spFactoryDiff.eq(expectedIncrease),
          'Expected increase not found in SPFactory'
        )
      })
      ...
      it('Slash cancels pending undelegate request', async () => {
        ...
        assert.isTrue(
          requestInfo.lockupExpiryBlock.gt(_lib.toBN(0)),
          'Expected lockup expiry block to be set'
        )
        assert.isTrue(
          requestInfo.amount.gt(_lib.toBN(0)),
          'Expected amount to be set'
        )
        ...
        assert.isTrue(
          info.totalInStakingContract.eq(_lib.toBN(0)),
          'No stake expected'
        )
      })
      ...
      it('Update decreaseStakeLockupDuration', async () => {
        let duration =
          await serviceProviderFactory.getDecreaseStakeLockupDuration()
        assert.equal(
          _lib.fromBN(duration),
          DECREASE_STAKE_LOCKUP_DURATION,
        ...
        let updateDecreaseStakeDurationTx =
          await governance.guardianExecuteTransaction(
            serviceProviderFactoryKey,
            _lib.toBN(0),
        ...
        await expectEvent.inTransaction(
          updateDecreaseStakeDurationTx.tx,
          ServiceProviderFactory,
        ...
        let tx = await serviceProviderFactory.requestDecreaseStake(
          DEFAULT_AMOUNT.div(_lib.toBN(2)),
          { from: stakerAccount }
        )
        let blocknumber = _lib.toBN(tx.receipt.blockNumber)
    ...
    it('Delegate ops after serviceType removal', async () => {
      ...
      assert.isTrue(
        spDetails.deployerStake.eq(DEFAULT_AMOUNT),
        'Expected deployerStake == default amount'
      )
      assert.isTrue(spDetails.validBounds, 'Expected validBounds == true')
      assert.isTrue(
        spDetails.numberOfEndpoints.eq(_lib.toBN(1)),
        'Expected one endpoint'
      )
      assert.isTrue(
        spDetails.minAccountStake.eq(minStakeBN),
        'Expected minAccountStake == dpTypeMin'
      )
      assert.isTrue(
        spDetails.maxAccountStake.eq(maxStakeBN),
        'Expected maxAccountStake == dpTypeMax'
      )
      ...
      assert.isTrue(
        totalStakedForSP.isZero(),
        'Expected totalStaked for SP == 0'
      )
      ...
      const totalLockedDelegationForSP =
        await delegateManager.getTotalLockedDelegationForServiceProvider(
          stakerAccount
        )
      assert.isTrue(
        totalLockedDelegationForSP.isZero(),
        'Expected totalLockedDelegationForSP == 0'
      )
    })
    ...
    it('lets a service provider return to valid bounds when a delegator changes delegation (only works with DelegateManagerV2)', async () => {
      ...
      assert.isTrue(
        totalStaked0.eq(DEFAULT_AMOUNT),
        'Expected totalStake == default amount'
      )
      assert.isTrue(
        spDetails0.deployerStake.eq(DEFAULT_AMOUNT),
        'Expected deployerStake == default amount'
      ...
      assert.isTrue(spDetails0.validBounds, 'Expected validBounds == true')
      assert.isTrue(
        totalStaked0.gt(spDetails0.minAccountStake),
        'Expected totalStake > minAccountStake'
      )
      assert.isTrue(
        totalStaked0.lt(spDetails0.maxAccountStake),
        'Expected totalStake < maxAccountStake'
      )
      ...
      assert.isTrue(
        totalStaked1.eq(DEFAULT_AMOUNT.add(delegationAmount)),
      ...
      assert.isTrue(
        totalStaked1.eq(spDetails1.maxAccountStake),
        'Expected totalStake == maxAccountStake'
      )
      ...
      assert.isTrue(
        totalStaked2.gt(spDetails2.maxAccountStake),
        'Expected totalStake > maxAccountStake'
      )
      ...
      assert.isTrue(
        totalStaked3.gt(spDetails1.minAccountStake),
        'Expected totalStake > minAccountStake'
      )
      assert.isTrue(
        totalStaked3.lt(spDetails1.maxAccountStake),
        'Expected totalStake < maxAccountStake'
      )
    ...
    it('Test proxy upgrade safety via openzeppelin tooling', async () => {
      ...
      const delManP1 = await deployProxy(DelegateManager, [
        token.address,
        governance.address,
        UNDELEGATE_LOCKUP_DURATION
      ])
      await upgradeProxy(delManP1, DelegateManagerV2)

      const delManP2 = await deployProxy(DelegateManager, [
        token.address,
        governance.address,
        UNDELEGATE_LOCKUP_DURATION
      ])
