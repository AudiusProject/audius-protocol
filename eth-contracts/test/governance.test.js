...
contract('Governance.sol', async (accounts) => {
  ...
  beforeEach(async function () {
    ...
    const serviceTypeManager0 = await ServiceTypeManager.new({
      from: proxyDeployerAddress
    })
    ...
    const serviceProviderFactory0 = await ServiceProviderFactory.new({
      from: proxyDeployerAddress
    })
    ...
    const serviceProviderFactoryProxy =
    ...
    let delegateManager0 = await DelegateManager.new({
      from: proxyDeployerAddress
    })
  ...
  beforeEach(async function () {
    ...
    await token.transfer(stakerAccount1, defaultStakeAmount, {
      from: tokenOwnerAddress
    })
    await token.transfer(stakerAccount2, defaultStakeAmount, {
      from: tokenOwnerAddress
    })
    await token.transfer(delegatorAccount1, defaultStakeAmount, {
      from: tokenOwnerAddress
    })
  ...
  it('Initialize require statements', async function () {
    ...
    const initializeArgumentTypesArray = [
      'address',
      'uint256',
      'uint256',
      'uint256',
  ...
  it('stakingAddress management', async function () {
    ...
    await registry2.addContract(governanceKey, governance2.address, {
      from: proxyDeployerAddress
    })
    ...
    await registry2.addContract(tokenRegKey, token2.address, {
      from: proxyDeployerAddress
    })
    ...
    const stakingInitializeData2 = _lib.encodeCall(
      'initialize',
      ['address', 'address'],
      [token2.address, governance2.address]
    )
    ...
    await registry2.addContract(stakingProxyKey, stakingProxy2.address, {
      from: proxyDeployerAddress
    })
    ...
    await _lib.assertRevert(
      governance2.setStakingAddress(staking2.address, {
        from: proxyDeployerAddress
      }),
      'revert'
    )
  ...
  it('ServiceProviderFactoryAddress management', async function () {
    ...
    const serviceProviderFactory2_0 = await ServiceProviderFactory.new({
      from: proxyDeployerAddress
    })
    ...
    const serviceProviderFactoryProxy2 =
      await AudiusAdminUpgradeabilityProxy.new(
    ...
    let serviceProviderFactory2 = await ServiceProviderFactory.at(
      serviceProviderFactoryProxy2.address
    )
    ...
    assert.equal(
      await governance.getServiceProviderFactoryAddress(),
      serviceProviderFactory2.address
    )
  })
  ...
  it('DelegateManagerAddress management', async function () {
    ...
    let delegateManager1 = await DelegateManager.new({
      from: proxyDeployerAddress
    })
    let delegateManagerProxy = await AudiusAdminUpgradeabilityProxy.new(
    ...
    let delegateManager2 = await DelegateManager.at(
      delegateManagerProxy.address
    )

    // Confirm delegateManager address cannot be set from non-governance address
    await _lib.assertRevert(
      governance.setDelegateManagerAddress(delegateManager2.address, {
        from: proxyDeployerAddress
      }),
      'revert'
    )
    ...
    assert.equal(
      await governance.getDelegateManagerAddress(),
  ...
  it('registryAddress management', async function () {
    ...
    await _lib.assertRevert(
      governance.setRegistryAddress(registry2.address, {
        from: proxyDeployerAddress
      }),
      'revert'
    )
    ...
    assert.equal(
      setRegistryAddressTx.event.args._newRegistryAddress,
      registry2.address,
      'Expected newRegistryAddress'
    )
  ...
  const getStakeInfo = async (address) => {
    ...
    let pendingDecreaseStakeReq =
      await serviceProviderFactory.getPendingDecreaseStakeRequest(address)
    let totalDelegatedStake = await delegateManager.getTotalDelegatorStake(
      address
    )
    let pendingUndelegateStakeReq =
      await delegateManager.getPendingUndelegateRequest(address)
    let totalStake = totalDeployerStake.add(totalDelegatedStake)
    let totalLockedUpStake = pendingDecreaseStakeReq.amount.add(
      pendingUndelegateStakeReq.amount
    )
  ...
  describe('Submit Proposal Active Stake Validation', async function () {
    ...
    let callData = _lib.abiEncode(
      ['uint256', 'address'],
      [_lib.fromBN(slashAmount), targetAddress]
    )
    ...
    beforeEach(async () => {
      ...
      await token.approve(staking.address, delegateAmount, {
        from: delegatorAccount1
      })
      // Delegate half default stake amount
      await delegateManager.delegateStake(stakerAccount1, delegateAmount, {
        from: delegatorAccount1
      })
    })
    ...
    it('Submit proposal via delegator address', async function () {
      let delegationFromContract = await delegateManager.getTotalDelegatorStake(
        delegatorAccount1
      )
      let totalStakedForDelegator = await staking.totalStakedFor(
        delegatorAccount1
      )
      assert.isTrue(
        delegationFromContract.eq(delegateAmount),
        'Expected equivalent value in contract'
      )
      assert.isTrue(
        totalStakedForDelegator.eq(_lib.toBN(0)),
        'No direct stake expected'
      )
    ...
    it('Submit proposal from delegator-only address w/partially undelegated stake', async function () {
      ...
      assert.isTrue(
        stakeInfo.totalActiveStake.gt(_lib.toBN(0)),
        'Expect some active stake'
      )
      assert.isTrue(
        stakeInfo.totalLockedUpStake.gt(_lib.toBN(0)),
        'Expect some locked stake'
      )
      await submitAndVerifySlashProposalSubmitted(delegatorAccount1)
    })
    ...
    it('Submit proposal from invalid address', async function () {
      ...
      const totalDelegated = await delegateManager.getTotalDelegatorStake(
        invalidAddress
      )
      const totalStaked = await staking.totalStakedFor(invalidAddress)
      const spInfo = await serviceProviderFactory.getServiceProviderDetails(
        invalidAddress
      )
    ...
    it('Submit proposal via service provider address w/no active stake', async function () {
      ...
      // Expected to fail
      await serviceProviderFactory.requestDecreaseStake(defaultStakeAmount, {
        from: stakerAccount1
      })
      let stakeInfo = await getStakeInfo(stakerAccount1)
      assert.isTrue(
        stakeInfo.totalActiveStake.eq(_lib.toBN(0)),
    ...
    it('Submit proposal from delegator + sp address w/partial active stake', async function () {
      await token.transfer(stakerAccount1, delegateAmount, {
        from: proxyDeployerAddress
      })
      await token.approve(staking.address, delegateAmount, {
        from: stakerAccount1
      ...
      await delegateManager.delegateStake(stakerAccount2, delegateAmount, {
        from: stakerAccount1
      })
      // Lock up partial funds in both spfactory, delegatemanager
      await serviceProviderFactory.requestDecreaseStake(
        defaultStakeAmount.div(_lib.toBN(2)),
        { from: stakerAccount1 }
      )
      await delegateManager.requestUndelegateStake(
        stakerAccount2,
        delegateAmount.div(_lib.toBN(3)),
        { from: stakerAccount1 }
      )
  ...
  describe('Proposal end-to-end test - slash action', async function () {
    it('Initial state - Ensure no Proposals exist yet', async function () {
      await _lib.assertRevert(
        governance.getProposalById(0),
        'Must provide valid non-zero _proposalId'
      )
      await _lib.assertRevert(
        governance.getProposalById(1),
        'Must provide valid non-zero _proposalId'
      )
    ...
    it('Should fail to submit Proposal for unregistered target contract', async function () {
      ...
      const callData = _lib.abiEncode(
        ['uint256', 'address'],
        [slashAmount.toNumber(), targetAddress]
      )
    ...
    it('Fail to submitProposal when an outstanding proposal exists', async function () {
      ...
      await time.advanceBlockTo(
        proposalStartBlockNumber + votingPeriod + executionDelay
      )
    ...
    it('Proposal description', async function () {
      ...
      assert.equal(
        tx.event.args._description,
        descriptionCorrect,
        'Expected same event.args.description'
      )
    })
    ...
    it('Submit Proposal for Slash', async function () {
      ...
      assert.equal(
        parseInt(txParsed.event.args._proposalId),
        proposalId,
        'Expected same event.args.proposalId'
      )
      assert.equal(
        txParsed.event.args._proposer,
        proposerAddress,
        'Expected same event.args.proposer'
      )
      assert.isTrue(
        parseInt(txReceipt.receipt.blockNumber) > lastBlock,
        'Expected submitProposalTx blockNumber > lastBlock'
      )
      assert.equal(
        txParsed.event.args._description,
        proposalDescription,
        'Expected same event.args.description'
      )
      ...
      assert.equal(
        parseInt(proposal.proposalId),
        proposalId,
        'Expected same proposalId'
      )
      ...
      assert.equal(
        _lib.toStr(proposal.targetContractRegistryKey),
        _lib.toStr(targetContractRegistryKey),
        'Expected same proposal.targetContractRegistryKey'
      )
      assert.equal(
        proposal.targetContractAddress,
        targetContractAddress,
        'Expected same proposal.targetContractAddress'
      )
      assert.equal(
        proposal.callValue.toNumber(),
        callValue0,
        'Expected same proposal.callValue'
      )
      assert.equal(
        proposal.functionSignature,
        functionSignature,
        'Expected same proposal.functionSignature'
      )
      assert.equal(
        proposal.callData,
        callData,
        'Expected same proposal.callData'
      )
      assert.equal(
        proposal.outcome,
        Outcome.InProgress,
      ...
      assert.equal(
        parseInt(proposal.voteMagnitudeYes),
        0,
        'Expected same voteMagnitudeYes'
      )
      assert.equal(
        parseInt(proposal.voteMagnitudeNo),
        0,
        'Expected same voteMagnitudeNo'
      )
      ...
      for (const account of accounts) {
        const { vote, voteMagnitude } =
          await governance.getVoteInfoByProposalAndVoter.call(
            proposalId,
            account
          )
    ...
    describe('Proposal voting', async function () {
      ...
      beforeEach(async function () {
        ...
        proposalStartBlockNumber = parseInt(
          submitProposalTxReceipt.receipt.blockNumber
        )
      })
      ...
      describe('Active stake validation', async function () {
        ...
        const submitAndVerifyActiveStakeVoteFailure = async (
          proposal,
          vote,
          voter
        ) => {
        ...
        const submitAndVerifyActiveStakeVoteSuccess = async (
          proposal,
          vote,
          voter,
          expectedVoteMagnitude
        ) => {
          const voteTxReceipt = await governance.submitVote(proposal, vote, {
            from: voter
          })
          await expectEvent.inTransaction(
            voteTxReceipt.tx,
          ...
          const { vote: voterVote, voteMagnitude } =
            await governance.getVoteInfoByProposalAndVoter(proposal, voter)
        ...
        it('Evaluate with no votes', async function () {
          ...
          let evaluateTxReceipt = await governance.evaluateProposalOutcome(
            proposalId,
            { from: stakerAccount1 }
          )
          await expectEvent.inTransaction(
        ...
        */
        it('Confirm quorum change after submission does not prevent proposal evaluation', async function () {
          let totalStakeAtProposalTime = await staking.totalStakedAt(
            _lib.toBN(proposalStartBlockNumber)
          )
          let proposal = await governance.getProposalById(proposalId)
          let totalVotedStake = proposal.voteMagnitudeYes.add(
            proposal.voteMagnitudeNo
          )
          let proposalParticipationPercent = totalVotedStake
            .mul(_lib.toBN(100))
            .div(totalStakeAtProposalTime)
          // Vote from stakerAccount1 with current stake
          const voteYes = Vote.Yes
          await governance.submitVote(proposalId, voteYes, {
            from: stakerAccount1
          })

          proposal = await governance.getProposalById(proposalId)
          totalVotedStake = proposal.voteMagnitudeYes.add(
            proposal.voteMagnitudeNo
          )
          proposalParticipationPercent = totalVotedStake
            .mul(_lib.toBN(100))
            .div(totalStakeAtProposalTime)

          // Increase stake from a new account by delegating
          let additionalDelegationAmount = defaultStakeAmount.mul(_lib.toBN(13))
          await token.transfer(delegatorAccount1, additionalDelegationAmount, {
            from: proxyDeployerAddress
          })
          await token.approve(staking.address, additionalDelegationAmount, {
            from: delegatorAccount1
          })
          await delegateManager.delegateStake(
            stakerAccount2,
            additionalDelegationAmount,
            { from: delegatorAccount1 }
          )

          let delegatorInfo = await getStakeInfo(delegatorAccount1)
          assert.isTrue(
            delegatorInfo.totalDeployerStake.eq(_lib.toBN(0)),
            'No deployer stake expected'
          )
          assert.isTrue(
            delegatorInfo.totalDelegatedStake.eq(
              delegateAmount.add(additionalDelegationAmount)
            ),
          ...
          proposalParticipationPercent = totalVotedStake
            .mul(_lib.toBN(100))
            .div(totalStakeAtProposalTime)
          let totalParticipationPercent = totalVotedStake
            .mul(_lib.toBN(100))
          ...
          const { vote: voterVote, voteMagnitude } =
            await governance.getVoteInfoByProposalAndVoter(
              proposalId,
              stakerAccount2
            )
          assert.isTrue(
            voteMagnitude.eq(staker2Info.totalDeployerStake),
            'Expect delegated stake exclusion'
          )

          // Increase stake from account 1
          let increaseAmount = defaultStakeAmount.mul(_lib.toBN(5))
          await token.transfer(stakerAccount1, increaseAmount, {
            from: proxyDeployerAddress
          })
          await token.approve(staking.address, increaseAmount, {
            from: stakerAccount1
          })
          await serviceProviderFactory.increaseStake(increaseAmount, {
            from: stakerAccount1
          })
          ...
          proposalParticipationPercent = totalVotedStake
            .mul(_lib.toBN(100))
            .div(totalStakeAtProposalTime)
          totalParticipationPercent = totalVotedStake
            .mul(_lib.toBN(100))
            .div(totalCurrentStake)
          // Confirm that all votes are < current quorum given totalStake
          assert.isTrue(
            totalParticipationPercent.lt(_lib.toBN(votingQuorumPercent)),
            'Total participation should be less than quorum'
          )
          // Confirm that all votes are > proposal expected quorum
          assert.isTrue(
            proposalParticipationPercent.gt(_lib.toBN(votingQuorumPercent)),
            'Total particpation from initial proposal submission should be greater than quorum'
          )
          await time.advanceBlockTo(
            proposalStartBlockNumber + votingPeriod + executionDelay
          )
        ...
        it('Submit vote from service provider only address w/partial active stake', async function () {
          const vote = Vote.Yes
          let decreaseAmount = defaultStakeAmount
            .div(_lib.toBN(2))
            .sub(_lib.toBN(1)) // (defaultStakeAmount / 2) + 1
          // Lock up partial funds in spfactory
          await serviceProviderFactory.requestDecreaseStake(decreaseAmount, {
            from: voter1Address
          })
          let stakeInfo = await getStakeInfo(voter1Address)
          await submitAndVerifyActiveStakeVoteSuccess(
            proposalId,
            vote,
            voter1Address,
          ...
          let proposal = await governance.getProposalById(proposalId)
          assert.isTrue(
            proposal.voteMagnitudeYes.eq(stakeInfo.totalActiveStake),
            `Expected voteMagnitudeYes: ${proposal.voteMagnitudeYes.toString()} to equal active stake ${stakeInfo.totalActiveStake.toString()}`
          )
          assert.isFalse(
            proposal.voteMagnitudeYes.eq(stakeInfo.totalStake),
            `Expected voteMagnitudeYes: ${proposal.voteMagnitudeYes.toString()} to NOT equal total stake ${stakeInfo.totalStake.toString()}`
          )
          let totalVotedStake = proposal.voteMagnitudeYes.add(
            proposal.voteMagnitudeNo
          )

          let minQuorumStake = stakeInfo.totalStake
            .mul(_lib.toBN(votingQuorumPercent))
            .div(_lib.toBN(100)) // (total stake * quorum %) / 100
          assert.isTrue(
            totalVotedStake.gt(minQuorumStake),
            'Total voted stake should be greater than quorum'
          )
        ...
        it('Submit vote from service provider only address w/partial active stake under quorum, execution will fail', async function () {
          const vote = Vote.Yes
          let decreaseAmount = defaultStakeAmount.sub(
            _lib.toBN(_lib.audToWei('1'))
          ) // reduce active stake to 1 aud from 1000
          // Lock up partial funds in spfactory
          await serviceProviderFactory.requestDecreaseStake(decreaseAmount, {
            from: voter1Address
          })
          ...
          let minQuorumStake = stakeInfo.totalStake
            ...
            .div(_lib.toBN(100)) // (total stake * quorum %) / 100
          assert.isTrue(
            totalVotedStake.lt(minQuorumStake),
            'Total voted stake should be less than quorum'
          )
        ...
        it('Submit vote from delegator-only address w/partially undelegated stake', async function () {
          ...
          assert.isTrue(stakeInfo.totalStake.eq(delegateAmount))
          assert.isTrue(
            stakeInfo.totalActiveStake.eq(delegateAmount.sub(undelegateAmount))
          )
          const voteTxReceipt = await governance.submitVote(proposalId, vote, {
            from: delegatorAccount1
          })
          await expectEvent.inTransaction(
            voteTxReceipt.tx,
        ...
        it('Submit vote from delegator-only address w/no active stake', async function () {
          ...
          await submitAndVerifyActiveStakeVoteFailure(
            proposalId,
            vote,
            delegatorAccount1
          )
        ...
        it('Update vote fails to increase magnitude past initial vote, even if new value has been staked by this address', async function () {
          ...
          assert.isTrue(
            stakeInfo2.totalStake.eq(
              stakeInfo1.totalDeployerStake.add(increaseAmount)
            ),
            'Mismatched state'
          )
          ...
          assert.isTrue(
            stakeInfo2.totalActiveStake
              .sub(stakeInfo1.totalActiveStake)
              .eq(increaseAmount),
            'Expect increase in active stake'
          )

          // Submit vote update
          // Confirm that voter stake is equal to the initial active, and note active prior to update
          const voteTx3 = await governance.updateVote(proposalId, voteYes, {
            from: stakerAccount1
          })
          await expectEvent.inTransaction(
            voteTx3.tx,
      ...
      it('Fail to vote after votingPeriod has ended', async function () {
        ...
        const proposalStartBlockNumber = parseInt(
          submitProposalTxReceipt.receipt.blockNumber
        )
      ...
      it('Fail to submit invalid vote', async function () {
        await _lib.assertRevert(
          governance.submitVote(proposalId, Vote.None, {
            from: stakerAccount1
          }),
      ...
      it('Successfully vote on Proposal for Slash', async function () {
        ...
        // Call submitVote()
        const txReceipt = await governance.submitVote(proposalId, vote, {
          from: voter1Address
        })
        ...
        assert.equal(
          txParsed.event.args._voter,
          voter1Address,
          'Expected same event.args.voter'
        )
        assert.equal(
          parseInt(txParsed.event.args._vote),
          vote,
          'Expected same event.args.vote'
        )
        assert.isTrue(
          txParsed.event.args._voterStake.eq(defaultStakeAmount),
          'Expected same event.args.voterStake'
        )
        ...
        assert.equal(
          proposal.proposer,
          proposerAddress,
          'Expected same proposer'
        )
        assert.isTrue(
          proposal.submissionBlockNumber > lastBlock,
          'Expected submissionBlockNumber > lastBlock'
        )
        assert.equal(
          _lib.toStr(proposal.targetContractRegistryKey),
          _lib.toStr(targetContractRegistryKey),
        ...
        assert.isTrue(
          proposal.callValue.eq(callValue),
          'Expected same proposal.callValue'
        )
        assert.equal(
          proposal.functionSignature,
        ...
        assert.isTrue(
          proposal.voteMagnitudeYes.isZero(),
          'Expected same voteMagnitudeYes'
        )
        assert.isTrue(
          proposal.voteMagnitudeNo.eq(defaultStakeAmount),
          'Expected same voteMagnitudeNo'
        )
      ...
      it('Successfully vote multiple times with diff accounts', async function () {
        ...
        // voter1 voteYes
        const voteTx1 = await governance.submitVote(proposalId, voteYes, {
          from: voter1Address
        })
        const voteTxParsed1 = _lib.parseTx(voteTx1)
        assert.equal(
          parseInt(voteTxParsed1.event.args._vote),
          voteYes,
          'Expected same event.args.vote'
        )

        // voter2 voteYes
        const voteTx2 = await governance.submitVote(proposalId, voteYes, {
          from: voter2Address
        })
        const voteTxParsed2 = _lib.parseTx(voteTx2)
        assert.equal(
          parseInt(voteTxParsed2.event.args._vote),
          voteYes,
          'Expected same event.args.vote'
        )
        ...
        assert.isTrue(
          proposal.voteMagnitudeYes.eq(defaultStakeAmount.mul(_lib.toBN(2))),
          'Expected same voteMagnitudeYes'
        )
        assert.isTrue(
          proposal.voteMagnitudeNo.isZero(),
          'Expected same voteMagnitudeNo'
        )
        assert.equal(parseInt(proposal.numVotes), 2, 'Expected same numVotes')

        // voter1 update to voteNo
        const voteTx3 = await governance.updateVote(proposalId, voteNo, {
          from: voter1Address
        })
        const voteTxParsed3 = _lib.parseTx(voteTx3)
        assert.equal(
          parseInt(voteTxParsed3.event.args._vote),
          voteNo,
          'Expected same event.args.vote'
        )
        assert.equal(
          parseInt(voteTxParsed3.event.args._previousVote),
          voteYes,
          'Expected same event.args.previousVote'
        )

        // voter1 update to voteYes
        const voteTx4 = await governance.updateVote(proposalId, voteYes, {
          from: voter1Address
        })
        const voteTxParsed4 = _lib.parseTx(voteTx4)
        assert.equal(
          parseInt(voteTxParsed4.event.args._vote),
          voteYes,
          'Expected same event.args.vote'
        )
        assert.equal(
          parseInt(voteTxParsed4.event.args._previousVote),
          voteNo,
          'Expected same event.args.previousVote'
        )

        // voter1 update to same
        const voteTx5 = await governance.updateVote(proposalId, voteYes, {
          from: voter1Address
        })
        const voteTxParsed5 = _lib.parseTx(voteTx5)
        assert.equal(
          parseInt(voteTxParsed5.event.args._vote),
          voteYes,
          'Expected same event.args.vote'
        )
        assert.equal(
          parseInt(voteTxParsed5.event.args._previousVote),
          voteYes,
          'Expected same event.args.previousVote'
        )
        ...
        // Confirm vote states
        const vote1Info = await governance.getVoteInfoByProposalAndVoter.call(
          proposalId,
          voter1Address
        )
        assert.equal(vote1Info.vote, voteYes)
        assert.isTrue(vote1Info.voteMagnitude.eq(defaultStakeAmount))
        const vote2Info = await governance.getVoteInfoByProposalAndVoter.call(
          proposalId,
          voter2Address
        )
      ...
      it('Reject a proposal with a tie', async function () {
        await governance.submitVote(proposalId, Vote.Yes, {
          from: stakerAccount1
        })
        await governance.submitVote(proposalId, Vote.No, {
          from: stakerAccount2
        })
        ...
        assert.isTrue(
          proposal.voteMagnitudeYes.eq(defaultStakeAmount),
          'Expected same voteMagnitudeYes'
        )
        assert.isTrue(
          proposal.voteMagnitudeNo.eq(defaultStakeAmount),
        ...
        assert.equal(
          txParsedEvent0.event.name,
          'ProposalOutcomeEvaluated',
          'Expected same event name'
        )
        assert.equal(
          parseInt(txParsedEvent0.event.args._proposalId),
          proposalId,
          'Expected same event.args.proposalId'
        )
        assert.equal(
          txParsedEvent0.event.args._outcome,
          Outcome.Rejected,
          'Expected same event.args.outcome'
        )
        assert.isTrue(
          txParsedEvent0.event.args._voteMagnitudeYes.eq(defaultStakeAmount),
          'Expected same event.args.voteMagnitudeYes'
        )
        assert.isTrue(
          txParsedEvent0.event.args._voteMagnitudeNo.eq(defaultStakeAmount),
          'Expected same event.args.voteMagnitudeNo'
        )
        assert.equal(
          parseInt(txParsedEvent0.event.args._numVotes),
          2,
          'Expected same event.args.numVotes'
        )
      })
    })
    ...
    describe('Proposal evaluation', async function () {
      ...
      beforeEach(async function () {
        ...
        assert.isTrue(
          initialTotalStake.eq(defaultStakeAmount.mul(_lib.toBN(2)))
        )
        ...
        await governance.submitVote(proposalId, voter1Vote, {
          from: voter1Address
        })
      ...
      it('Fail to evaluate proposal before votingPeriod and executionDelay have ended', async function () {
        ...
        await governance.evaluateProposalOutcome(proposalId, {
          from: proposerAddress
        })
        ...
        proposalId = _lib.parseTx(submitProposalTxReceipt).event.args
          ._proposalId

        await _lib.assertRevert(
          governance.evaluateProposalOutcome(proposalId, {
            from: proposerAddress
          }),
      ...
      it('Confirm proposal evaluated correctly + transaction executed', async function () {
        ...
        const [txParsedEvent0, txParsedEvent1] = _lib.parseTx(
          evaluateTxReceipt,
          true
        )
        assert.equal(
        ...
        assert.equal(
          parseInt(txParsedEvent0.event.args._proposalId),
          proposalId,
          'Expected same txParsedEvent0.event.args.proposalId'
        )
        assert.equal(
          txParsedEvent0.event.args._success,
          true,
          'Expected same txParsedEvent0.event.args.returnData'
        )
        assert.equal(
          txParsedEvent0.event.args._returnData,
          returnData,
          'Expected same txParsedEvent0.event.args.returnData'
        )
        assert.equal(
          txParsedEvent1.event.name,
          'ProposalOutcomeEvaluated',
          'Expected same event name'
        )
        assert.equal(
          parseInt(txParsedEvent1.event.args._proposalId),
          proposalId,
          'Expected same event.args.proposalId'
        )
        assert.equal(
          txParsedEvent1.event.args._outcome,
          outcome,
          'Expected same event.args.outcome'
        )
        assert.isTrue(
          txParsedEvent1.event.args._voteMagnitudeYes.eq(defaultStakeAmount),
          'Expected same event.args.voteMagnitudeYes'
        )
        assert.isTrue(
          txParsedEvent1.event.args._voteMagnitudeNo.isZero(),
          'Expected same event.args.voteMagnitudeNo'
        )
        assert.equal(
          parseInt(txParsedEvent1.event.args._numVotes),
          1,
          'Expected same event.args.numVotes'
        )
        ...
        assert.isTrue(
          parseInt(proposal.submissionBlockNumber) > lastBlock,
          'Expected submissionBlockNumber > lastBlock'
        )
        assert.equal(
          _lib.toStr(proposal.targetContractRegistryKey),
        ...
        assert.equal(
          _lib.fromBN(proposal.callValue),
          callValue,
          'Expected same proposal.callValue'
        )
        assert.equal(
        ...
        const totalStakeAtSubmission = await staking.totalStakedAt.call(
        ...
        const participationPercent =
          (totalVotedStake / totalStakeAtSubmission) * 100
        assert.isAtLeast(
        ...
        assert.isTrue(
          _lib
            .toBN(initialTotalStake)
            .sub(_lib.toBN(slashAmount))
            .eq(await staking.totalStaked()),
      ...
      it('Proposal with Outcome.Rejected', async function () {
        ...
        assert.equal(
          txParsed.event.args._outcome,
          outcome,
          'Expected same event.args.outcome'
        )
        assert.isTrue(
          txParsed.event.args._voteMagnitudeYes.isZero(),
          'Expected same event.args.voteMagnitudeYes'
        )
        assert.isTrue(
          txParsed.event.args._voteMagnitudeNo.eq(
            defaultStakeAmount.mul(THREE).div(TWO)
          ),
          'Expected same event.args.voteMagnitudeNo'
        )
        assert.isTrue(
          txParsed.event.args._numVotes.eq(TWO),
          'Expected same event.args.numVotes'
        )
        ...
        assert.isTrue(
          proposal.voteMagnitudeNo.eq(defaultStakeAmount.mul(THREE).div(TWO)),
          'Expected same voteMagnitudeNo'
        )
      ...
      it('Confirm voting quorum restriction is enforced', async function () {
        ...
        assert.isTrue(
          txParsed.event.args._voteMagnitudeNo.isZero(),
          'Expected same event.args.voteMagnitudeNo'
        )
        assert.isTrue(
          txParsed.event.args._numVotes.isZero(),
          'Expected same event.args.numVotes'
        )
        ...
        const proposalId2 = _lib.parseTx(submitProposalTxReceipt2).event.args
          ._proposalId
        await governance.submitVote(proposalId2, Vote.Yes, {
          from: voter1Address
        })

        // Confirm proposal would meet quorum
        let proposal2 = await governance.getProposalById.call(proposalId2)
        const totalVotedStake2 =
          parseInt(proposal2.voteMagnitudeNo) +
          parseInt(proposal2.voteMagnitudeYes)
        // div before mul bc js does large number math incorrectly
        let participationPercent2 =
          (totalVotedStake2 / totalStakeAtSubmission) * 100
        let latestVotingQuorumPercent = parseInt(
          await governance.getVotingQuorumPercent.call()
        )
        assert.isAtLeast(
          participationPercent2,
          latestVotingQuorumPercent,
          'Quorum would be met'
        )
        ...
        // Advance blocks to the evaluatable block
        const proposal2StartBlockNumber = parseInt(
          submitProposalTxReceipt2.receipt.blockNumber
        )
        await time.advanceBlockTo(
          proposal2StartBlockNumber + votingPeriod + executionDelay
        )
        ...
        assert.equal(
          proposal2New.outcome,
          outcome,
          'Expected QuorumNotMet outcome'
        )

        // Confirm quorum was correctly calculated
        const totalVotedStake2New =
          parseInt(proposal2New.voteMagnitudeYes) +
          parseInt(proposal2New.voteMagnitudeNo)
        // div before mul bc js does large number math incorrectly
        const participationPercent2New =
          (totalVotedStake2New / totalStakeAtSubmission) * 100
        latestVotingQuorumPercent = parseInt(
          await governance.getVotingQuorumPercent.call()
        )
        assert.isBelow(
          participationPercent2New,
          latestVotingQuorumPercent,
          'Quorum not met'
        )
      })
      ...
      it('evaluateProposal fails after targetContract has been upgraded', async function () {
        ...
        const outcomeTargetContractAddressChanged =
        ...
        const txR = await governance.evaluateProposalOutcome(proposalId, {
          from: proposerAddress
        })
        ...
        assert.equal(
          parseInt(tx.event.args._proposalId),
          proposalId,
          'Expected same event.args.proposalId'
        )
        assert.equal(
          tx.event.args._outcome,
          outcomeTargetContractAddressChanged,
          'Expected same event.args.outcome'
        )
        assert.isTrue(
          tx.event.args._voteMagnitudeYes.eq(defaultStakeAmount),
          'Expected same event.args.voteMagnitudeYes'
        )
        assert.isTrue(
          tx.event.args._voteMagnitudeNo.isZero(),
          'Expected same event.args.voteMagnitudeNo'
        )
        assert.equal(
          parseInt(tx.event.args._numVotes),
          1,
          'Expected same event.args.numVotes'
        )
      ...
      it('Call evaluateProposal where transaction execution fails', async function () {
        ...
        await serviceProviderFactory.requestDecreaseStake(decreaseStakeAmount, {
          from: stakerAccount2
        })
        let requestInfo =
        ...
        const decreasedStakeAcct2 = await staking.totalStakedFor.call(
          stakerAccount2
        )
        assert.isTrue(
          decreasedStakeAcct2.eq(initialStakeAcct2.sub(decreaseStakeAmount))
        )
        ...
        assert.equal(
          txParsedEvent0.event.args._proposalId,
          proposalId,
          'Expected same txParsedEvent0.event.args.proposalId'
        )
        assert.equal(
          txParsedEvent0.event.args._success,
          false,
          'Expected same txParsedEvent0.event.args.success'
        )
        ...
        assert.isTrue(
          (await staking.totalStaked()).eq(
            initialTotalStake.sub(decreaseStakeAmount)
          ),
          'Expected total stake amount to be unchanged'
        )
        assert.isTrue(
          (await token.totalSupply()).eq(initialTokenSupply),
          'Expected total token supply to be unchanged'
        )
      })
      ...
      describe('Veto logic', async function () {
        ...
        it('Fail to veto proposal with invalid proposalId', async function () {
          ...
          await _lib.assertRevert(
            governance.vetoProposal(invalidProposalId, {
              from: guardianAddress
            }),
            'Must provide valid non-zero _proposalId.'
          )
        ...
        it('Successfully veto proposal + ensure further actions on proposal are blocked', async function () {
          const vetoTxReceipt = await governance.vetoProposal(proposalId, {
            from: guardianAddress
          })
          ...
          assert.equal(
            parseInt(vetoTx.event.args._proposalId),
            proposalId,
          ...
          await _lib.assertRevert(
            governance.submitVote(proposalId, voter1Vote, {
              from: voter1Address
            }),
  ...
  it('Proposal end-to-end test - upgrade contract action', async function () {
    ...
    await _lib.assertRevert(
      stakingCopy.newFunction.call({ from: proxyDeployerAddress }),
      'revert'
    )

    // Deploy new logic contract to later upgrade to
    const stakingUpgraded0 = await StakingUpgraded.new({
      from: proxyAdminAddress
    })
    ...
    await time.advanceBlockTo(
      proposalStartBlock + votingPeriod + executionDelay
    )

    // Call evaluateProposalOutcome()
    const evaluateTxReceipt = await governance.evaluateProposalOutcome(
      proposalId,
      { from: proposerAddress }
    )
    ...
    assert.equal(
      parseInt(txParsedEvent0.event.args._proposalId),
      proposalId,
      'Expected event.args.proposalId'
    )
    assert.equal(
      txParsedEvent0.event.args._success,
      true,
      'Expected event.args.returnData'
    )
    assert.equal(
      txParsedEvent0.event.args._returnData,
      returnData,
    ...
    const newFnResp = await stakingCopy2.newFunction.call({
      from: proxyDeployerAddress
    })
  ...
  it('Contract content change prevents proposal evaluation', async function () {
    ...
    const mockAccountContract = await MockAccount.new(owner, {
      from: proxyDeployerAddress
    })
    const accountKey = web3.utils.utf8ToHex('Account')
    await registry.addContract(accountKey, mockAccountContract.address, {
      from: proxyDeployerAddress
    })
    ...
    let proposalContractHash = await governance.getProposalTargetContractHash(
      proposalId
    )
    ...
    assert.isTrue(
      txParsedEvent0.event.args._voteMagnitudeNo.isZero(),
      'Expected same event.args.voteMagnitudeNo'
    )
    assert.equal(
      parseInt(txParsedEvent0.event.args._numVotes),
    ...
    let proposalContractHashAfterEvaluation =
      await governance.getProposalTargetContractHash(proposalId)
    assert.equal(
      proposalContractHash,
      proposalContractHashAfterEvaluation,
      'Expect same proposal hash despite target contract diff'
    )

    // Additional coverage for getProposalTargetContractHash
    await _lib.assertRevert(
      governance.getProposalTargetContractHash(0),
      'Must provide valid non-zero _proposalId'
    )
    await _lib.assertRevert(
      governance.getProposalTargetContractHash(10000),
      'Must provide valid non-zero _proposalId'
    )
  })
  ...
  it('Test max value of maxInProgressProposals', async function () {
    ...
    assert.isTrue(
      await governance.inProgressProposalsAreUpToDate.call(),
      'Expected all proposals to be uptodate'
    )
    ...
    assert.isFalse(
      await governance.inProgressProposalsAreUpToDate.call(),
      'Expected all proposals to not be uptodate'
    )

    for (let i = 1; i <= newMaxInProgressProposals; i++) {
      const evaluateTxR = await governance.evaluateProposalOutcome(i, {
        from: proposerAddress
      })
    }
  ...
  describe('Guardian execute transactions', async function () {
    ...
    it('Slash staker', async function () {
      ...
      assert.equal(
        guardianExecTx.event.name,
        'GuardianTransactionExecuted',
        'event.name'
      )
      assert.equal(
        guardianExecTx.event.args._targetContractAddress,
        targetContractAddress,
        'event.args.targetContractAddress'
      )
      assert.isTrue(
        guardianExecTx.event.args._callValue.eq(callValue),
      ...
      assert.equal(
        guardianExecTx.event.args._returnData,
        returnData,
      ...
      // Confirm Slash action succeeded by checking new Stake + Token values
      const finalStakeAcct2 = await staking.totalStakedFor(targetAddress)
      assert.isTrue(finalStakeAcct2.eq(defaultStakeAmount.sub(slashAmount)))
      assert.isTrue(
    ...
    it('Update proxy admin address', async function () {
      let stakingProxy = await AudiusAdminUpgradeabilityProxy.at(
        staking.address
      )
    ...
    it('Upgrade governance contract', async function () {
      ...
      await _lib.assertRevert(
        governanceCopy.newFunction.call({ from: proxyDeployerAddress }),
        'revert'
      )
      ...
      const isGovernanceAddress =
        await governanceCopy2.isGovernanceAddress.call({
          from: proxyDeployerAddress
        })
      assert.isTrue(
        isGovernanceAddress,
        'Contract should identity self as governance'
      )
      const newFnResp = await governanceCopy2.newFunction.call({
        from: proxyDeployerAddress
      })
      assert.equal(newFnResp, 5)

      // Confirm that proxy contract's implementation address has upgraded
      const govProxy = await AudiusAdminUpgradeabilityProxy.at(
        governance.address
      )
      assert.equal(
    ...
    it('Transfer guardianship', async function () {
      ...
      assert.equal(
        await governance.getGuardianAddress(),
        guardianAddress,
        'Expected same guardianAddress'
      )
      await governance.guardianExecuteTransaction(
        serviceTypeManagerProxyKey,
        callValue0,
        ...
        _lib.abiEncode(
          ['bytes32', 'bytes32'],
          [testDiscProvType, serviceVersion1]
        ),
        { from: guardianAddress }
      )
      ...
      await _lib.assertRevert(
        governance.guardianExecuteTransaction(
          ...
          _lib.abiEncode(
            ['bytes32', 'bytes32'],
            [testDiscProvType, serviceVersion2]
          ),
      ...
      await _lib.assertRevert(
        governance.transferGuardianship(newGuardianAddress, {
          from: accounts[18]
        }),
        'Only guardian.'
      )
      ...
      let transferGuardianshipTx = await governance.transferGuardianship(
        newGuardianAddress,
        { from: guardianAddress }
      )
      ...
      assert.equal(
        transferGuardianshipTx.event.args._newGuardianAddress,
        newGuardianAddress,
        'Expected newGuardianAddress'
      )
  ...
  describe('Token governance', async function () {
    ...
    it('Token transfer via governance', async function () {
      ...
      assert.isTrue(
        (await token.balanceOf(governance.address)).eq(_lib.toBN(100))
      )
      ...
      // confirm balances
      assert.isTrue(
        (await token.balanceOf(governance.address)).eq(_lib.toBN(50))
      )
      ...
      assert.isTrue((await token.balanceOf(governance.address)).isZero())
      assert.isTrue(
        (await token.balanceOf(testContract.address)).eq(_lib.toBN(50))
      )
    })
    ...
    it.skip('TODO - Upgrade token', async function () {
      ...
      const tokenUpgraded0 = await AudiusToken.new({
        from: proxyDeployerAddress
      })

      // Ensure proxyAdminAddress no longer has any ability to upgrade
      await _lib.assertRevert(
        tokenProxy.upgradeTo(tokenUpgraded0.address, {
          from: proxyAdminAddress
        }),
  ...
  describe('Registry governance', async function () {
    beforeEach(async function () {
      ...
      await registry.addContract(registryRegKey, registry.address, {
        from: proxyDeployerAddress
      })
    })
    ...
    it('Modify registry via governance', async function () {
      ...
      assert.equal(
        await registry.getContract.call(contractRegKey),
        _lib.addressZero
      )
      ...
      await _lib.assertRevert(
        governance.guardianExecuteTransaction(
          ...
          _lib.abiEncode(
            ['bytes32', 'address'],
            [contractRegKey, contract.address]
          ),
          { from: guardianAddress }
        ),
        'Transaction failed.'
      ...
      // Confirm test contract is not yet registered
      assert.equal(
        await registry.getContract.call(newContractKey),
        _lib.addressZero
      )
      ...
      // Call evaluateProposalOutcome()
      await governance.evaluateProposalOutcome(proposalId0, {
        from: proposerAddress
      })
    ...
    it('Upgrade registry', async function () {
      ...
      // Deploy new logic contract to later upgrade to
      const registryUpgraded0 = await Registry.new({
        from: proxyDeployerAddress
      })
      ...
      await registryProxy.setAudiusProxyAdminAddress(governance.address, {
        from: proxyAdminAddress
      })
      ...
      assert.equal(
        await registry.getContract.call(registryRegKey),
  ...
  it('Upgrade governance contract via proposal + ensure upgrade safety', async function () {
    ...
    const govLogicAddress = await govProxy.implementation.call({
      from: proxyAdminAddress
    })
    ...
    const proposalId0 = _lib.parseTx(submitProposalTx0Receipt).event.args
      ._proposalId
    // Submit proposal vote for Yes
    await governance.submitVote(proposalId0, Vote.Yes, { from: voterAddress })
    // Advance blocks to after proposal evaluation period + execution delay
    const proposal0StartBlock = parseInt(
      submitProposalTx0Receipt.receipt.blockNumber
    )
    await time.advanceBlockTo(
      proposal0StartBlock + votingPeriod + executionDelay
    )
    // Call evaluateProposalOutcome()
    await governance.evaluateProposalOutcome(proposalId0, {
      from: proposerAddress
    ...
    const proposalId1 = _lib.parseTx(submitProposalTx1Receipt).event.args
      ._proposalId
    assert.isTrue(
      proposalId1.eq(proposalId0.add(_lib.toBN(1))),
      'proposalId should increment by 1'
    )

    // Deploy new logic contract to later upgrade to
    const governanceV2Logic = await GovernanceV2.new({
      from: proxyDeployerAddress
    })
    ...
    assert.isTrue(
      proposalId.eq(proposalId1.add(_lib.toBN(1))),
      'proposalId should not reset after governance contract proxy upgrade 😅'
    )
    ...
    const proposalStartBlock = parseInt(submitTxReceipt.receipt.blockNumber)
    await time.advanceBlockTo(
      proposalStartBlock + newVotingPeriod + executionDelay
    )
