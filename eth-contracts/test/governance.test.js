...
contract('Governance.sol', async (accounts) => {
  ...
  describe('Slash proposal', async () => {
    ...
    describe('Proposal evaluation', async () => {
      let proposalId, proposerAddress, slashAmount, targetAddress, voterAddress, vote, defaultVote
      ...
      beforeEach(async () => {
        ...
        callValue = bigNumberify(0)
        signature = 'slash(uint256,address)'
        callData = abiEncode(['uint256', 'address'], [slashAmount, targetAddress])
        outcome = Outcome.Yes 
        txHash = keccak256(
          abiEncode(
        ...
        initialTotalStake = parseInt(await staking.totalStaked())
        assert.equal(initialTotalStake, defaultStakeAmount * 2)
        initialStakeAcct2 = parseInt(await staking.totalStakedFor(targetAddress))
      ...
      it.only('Confirm proposal evaluated correctly + transaction executed', async () => {  
      ...
      it.only('Confirm Repeated evaluateProposal call fails', async () => {
        // Call evaluateProposalOutcome()
        evaluateTxReceipt = await governance.evaluateProposalOutcome(proposalId, { from: proposerAddress })
  ...
  describe.skip('Submit propopsal to upgrade contract', async () => {
