...
contract('Registry', async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    assert.equal(
      await registryProxy.getAudiusProxyAdminAddress.call(),
      proxyAdminAddress
    )
  })
  ...
  it('Should fail to register a non-contract address', async () => {
    ...
    await _lib.assertRevert(
      registry.addContract(contractName, nonContractAddress)
    )
  })
  ...
  it('Should add newly deployed contract to Registry', async () => {
    ...
    assert.equal(
      _lib.toStr(txInfo.event.args._name),
      _lib.toStr(contractName),
  ...
  it('Fail to add contract under already registered key', async () => {
    ...
    await _lib.assertRevert(
      registry.addContract(contractName, testContract2.address, {
        from: proxyDeployerAddress
      }),
  ...
  it('Fail to add register 0 address', async () => {
    await _lib.assertRevert(
      registry.addContract(contractName, _lib.addressZero, {
        from: proxyDeployerAddress
      }),
  ...
  it('Fail to upgrade unregistered contract', async () => {
    ...
    await _lib.assertRevert(
      registry.upgradeContract(contractName, testContract.address, {
        from: proxyDeployerAddress
      }),
  ...
  it('Fail to upgrade contract with zero address', async () => {
    ...
    await _lib.assertRevert(
      registry.upgradeContract(contractName, _lib.addressZero, {
        from: proxyDeployerAddress
      }),
  ...
  it('Should remove registered contract', async () => {
    ...
    assert.equal(
      _lib.toStr(removeTxInfo.event.args._name),
      _lib.toStr(contractName),
      'Expected same contract name'
    )
    assert.equal(
  ...
  it('Should upgrade registered contract', async () => {
    ...
    assert.equal(
      _lib.toStr(upgradeTxInfo.event.args._name),
      _lib.toStr(contractName),
      'Expected same contract name'
    )
    assert.equal(
