...
const main = async () => {
    ...
    for (let serviceName of serviceNames) {
        const callData = [`${web3.utils.utf8ToHex(serviceName.new_name)}`, `${web3.utils.utf8ToHex(process.env.RELEASE_VERSION)}`]
        ...
        url += encodeURIComponent(`@audius/${serviceName.legacy_name}@${process.env.RELEASE_VERSION}`)
        ...
        console.log(`Created ${serviceName.new_name} proposal for ${functionSignature}: ${proposalId}`)
